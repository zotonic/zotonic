...
.. glossary::
   ...
   Zotonic applications
        The directory in which user-installed Zotonic sites and modules are placed. Defaults to the path ``_checkouts`` relative to the Zotonic installation, but can be adjusted by changing the ``ZOTONIC_APPS`` environment variable. See :ref:`guide-deployment-env`.
