...
``lib/images/flags/``
	A collection of country flags. They are code using the two letter ISO-3166 code of the country. 
	For example :file:`lib/images/country/flag-nl.png` refers to the Dutch flag.
...
``lib/images/mimeicons/``
	Zotonic can’t generate a preview of all files. For the ones it can’t generate a preview an icon
...
``lib/images/noun/``
	...
	This collection is licensed with the Creative Commons Attribution 3.0 Unported (CC BY 3.0) License.
...
``lib/images/social/round/``
	...
	This means you are not allowed to sell these icons.
...
Most of the icons are in 256x256 PNG format.  That is too large for normal usage. Best is to resize the images in your templates using the :ref:`image template tag <tag_image>`.
