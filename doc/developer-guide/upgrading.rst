...
* The ``user_sites_dir`` and ``user_modules_dir`` configurations have been removed.
  The default location for sites and modules is now the ``_checkouts`` directory.
  ...
  To upgrade, move your ``user/modules`` and ``user/sites`` applications to the ``_checkouts``
  directory.
