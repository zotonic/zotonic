...
`tag#all_include` and `tag#catinclude`.
:::
...
| anondo    | Include the template as an anonymous user. Any ACL checks through `model#acl` in the included template will be executed as if the visitor was not logged in. | `anondo`       |
