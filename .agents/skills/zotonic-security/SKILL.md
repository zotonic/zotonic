...
- Be extra careful with identity checks such as `UserId =:= z_acl:user(Context)`. `z_acl:user/1` returns `undefined` for an anonymous context, so the check also succeeds when `UserId` is `undefined`. Use this comparison only when `UserId` is known to be a valid resource id or the `Context` is known to be authenticated; otherwise validate the id or require authentication explicitly before comparing.
