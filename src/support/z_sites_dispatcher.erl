...
dispatch_match(Tokens, Context) ->
    ...
    try
        Tokens1 = [ z_convert:to_binary(Token) || Token <- Tokens ],
        dispatch_compiler:match(Module, Tokens1, Context)
    catch 
        error:undef ->
            fail
    end.
...
split_path(Path) ->
    ...
    Parts3 = [ z_convert:to_list(Part) || Part <- Parts2 ],
    case lists:last(Parts3) of
        [] -> {lists:sublist(Parts3, length(Parts3)-1), true};
        _ -> {Parts3, false}    
    end.
