...
get_dirty(Key) ->
    case proplists:get_value(Key, ensure_config()) of
        undefined -> 
            case default(Key) of
                {ok, Value} -> 
                    case Key of
                        password -> write_config(Key, Value);
                        _ -> nop
                    end,
                    Value;
                undefined ->
...
handle_cast({set, Key, Value}, State) ->
    case proplists:get_value(Key, State#state.config) of
        ...
        _ -> 
            ...
            {no_reply, State#state{config=ensure_config()}}
    end;
...
write_config(Key, Value) ->
    Config = [{Key,Value}, {modify_date, erlang:localtime()}
                | proplists:delete(modify_date, proplists:delete(Key,ensure_config()))],
    write_config(Config).
        ...
        undefined ->
            case default(Prop) of
                {ok, Value} ->
                    case Prop of
                        password ->
                            write_config([{Prop, Value} | State#state.config]),
                            {reply, Value, State#state{config=ensure_config()}};
                        _ ->
                            {reply, Value, State}
                    end;
                undefined ->
