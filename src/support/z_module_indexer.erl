...
find(What, Name, Context) when is_binary(Name) ->
    find(What, binary_to_list(Name), Context);
...
scan(Context) ->
    ...
    [
        ...
        || What <- [ lib, template, scomp, action, validator, model, service ]
    ].
...
%% @doc Order function for #mfile records, compares module priority
mfile_compare(#mfile{prio=A}, #mfile{prio=B}) -> A =< B.
...
reindex_ets_lookup(State) ->
    ...
    Now = os:timestamp(),
    to_ets(State#state.templates, template, Now, Site),
