...
ssl_listener_options() ->
    {ok, CertOptions} = ensure_self_signed(zotonic_status),
    [
...
get_self_signed_files(Site) ->
    SSLDir = filename:join([site_dir(Site), "ssl", "self-signed"]),
    Options = [
...
site_dir(Site) ->
    case lists:member(Site, z_sites_manager:get_builtin_sites()) of
        true -> z_utils:lib_dir(priv);
        false ->
            case z_path:site_dir(Site) of
                undefined -> z_utils:lib_dir(priv);
                Path -> Path
            end
    end.
