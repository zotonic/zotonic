...
handle_cast({set, Props}, Session) ->
    Props1 = lists:foldl(fun({K,V}, Ps) ->
                            prop_replace(K, V, Ps, Session#session.context#context.site)
                         end,
...
handle_call({spawn_link, Module, Func, Args}, _From, Session) ->
    ...
    exometer:update([zotonic, Session#session.context#context.site, session, page_processes], 1),
...
handle_set(Key, Value, Session) ->
    Session#session{ props = prop_replace(Key, Value, Session#session.props, Session#session.context#context.site)}.
...
page_start(Context) ->
    ...
    case z_session_page:start_link(self(), PageId, Context) of
        {ok,PagePid} ->
            ...
            exometer:update([zotonic, Context#context.site, session, page_processes], 1),
