...
{% else %}
<p style="color: #666">{_ You received this mail because you are subscribed to the mailing list _} <a href="https://{{ m.site.hostname }}{{ list_id.page_url }}">{{ list_id.title }}</a>.
{% if recipient_id %}
{#	{{ recipient_id }}
...
{% else %}
	{% if m.mailinglist.subscription[list_id][email] as sub %}
		<a href="https://{{ m.site.hostname }}{% url mailinglist_unsubscribe confirm_key=sub.confirm_key %}">{_ Please unsubscribe _}</a> {_ when you don't want to receive any further mail from this list. _}
	{% endif %}
{% endif %}
</p>
{% endif %}
