...
{% elseif m.acl.user and zotonic_dispatch == `logon_change` %}
    {% wire id="password_reset" type="submit" postback={change user_id=m.acl.user} delegate=`controller_logon` %}
    <form id="password_reset" method="post" action="postback" class="z_logon_form">
        {% include form_fields_tpl username=username is_password_change %}
        {% javascript %}
            setTimeout(function(){
                z_init_postback_forms();
            }, 100);
