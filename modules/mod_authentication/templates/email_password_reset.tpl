...
{% if not id %}
    ...
    <p>{_ You tried to reset your password for _} <a href="{{ m.site.protocol }}://{{ m.site.hostname }}/">{{ m.site.hostname }}</a>.</p>
    ...
    <p>{_ If you think you do have an account, please try another email address. _}</p>
{% else %}
    <p>{_ Hello _} {{ m.rsc[id].name_first|default:m.rsc[id].title }},</p>

    <p>{_ You tried to reset your password for _} <a href="{{ m.site.protocol }}://{{ m.site.hostname }}/">{{ m.site.hostname }}</a>. {_ Below are your account details and a link to set a new password. _}</p>
...
<p>{_ If you didn't try to reset your password, you can safely ignore this email. Maybe someone made an error typing his or her email address. _}</p>
