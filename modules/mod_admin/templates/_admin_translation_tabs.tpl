...
<ul class="nav nav-tabs language-tabs">
    ...
    <li class="tab-{{ code }} {% if code == edit_language %}active{% endif %}"
        {% if not (code|member:r_language and (lang.is_enabled or lang.is_editable)) %}style="display: none"{% endif %}
        data-index="{{ forloop.counter0 }}" {% include "_language_attrs.tpl" language=code %}><a href="#{{ prefix }}-{{ code }}" data-toggle="tab">{{ lang.language|default:z_language }}</a></li>
    {% endfor %}
</ul>
{% endwith %}
{% endwith %}
