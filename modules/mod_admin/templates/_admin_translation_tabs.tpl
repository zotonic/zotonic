...
<ul class="nav nav-tabs language-tabs">
    ...
    <li class="tab-{{ code }} {% if code == edit_language %}active{% endif %}" {% if not code|member:r_language or not lang.is_enabled %}style="display: none"{% endif %} data-index="{{ forloop.counter0 }}" {% include "_language_attrs.tpl" language=code %}><a href="#{{ prefix }}-{{ code }}" data-toggle="tab">{{ lang.name|default:z_language }}</a></li>
    {% endfor %}
</ul>
{% endwith %}
{% endwith %}
