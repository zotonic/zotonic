...
<p class="address">
    ...
    {% if id.phone %}<a href="tel:{{ id.phone }}"><i class="glyphicon glyphicon-comment"></i> {{ id.phone }}</a><br/>{% endif %}
    {% if id.email %}<a href="mailto:{{ id.email }}"><i class="glyphicon glyphicon-envelope"></i> {{ id.email }}</a><br/>{% endif %}
    {% if id.website %}<a href="{{ id.website }}"><i class="glyphicon glyphicon-bookmark"></i> {{ id.website }}</a><br/>{% endif %}
...
{% elseif id.computed_location_lat %}
    <p class="btn btn-default"><a href="{% url map id=id %}">{_ Show map _}</a></p>
{% endif %}
