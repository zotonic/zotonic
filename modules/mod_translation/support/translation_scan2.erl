...
%% Parse an Erlang syntax tree
parse_syntax_tree(L, Acc) when is_list(Acc) ->
    lists:foldl(
            fun (X, Acc1) ->
                parse_syntax_tree(X, Acc1)
            end,
            Acc,
            L);
parse_syntax_tree(Node, Acc) ->
    case erl_syntax:type(Node) of
        attribute ->
            Acc;
        function ->
            Func = erl_syntax:data(Node),
            parse_syntax_tree(erl_syntax:function_clauses(Func), Acc);
        clause ->
            Clause = erl_syntax:data(Node),
            parse_syntax_tree(erl_syntax:clause_body(Clause), Acc);
        % match_expr ->
        %     Body = erl_syntax:
        application ->
            case erl_syntax:application_operator(Node) of
                {module_qualifier, {atom, _, z_trans}, {atom, _, trans}} ->
                    case erl_syntax:application_arguments(Node) of
                        [{string, _, S}|_] -> [S|Acc];
                        _ -> Acc
                    end;
                _ ->
                    parse_syntax_tree(erl_syntax:application_arguments(Node), Acc)
            end;
        macro ->
            case erl_syntax:macro_name(Node) of
                "__" ->
