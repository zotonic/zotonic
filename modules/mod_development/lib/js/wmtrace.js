...
function initDetailPanels() {
    ...
    decisionCalls.onchange = function() {
        ...
        if (val) {
            ...
            if (call.output != "wmtrace_not_exported") {
                ...
                callInput.innerHTML = htmlesc(call.input);
                if (call.output != null) {
                    callOutput.style.color = '#000000';
                    callOutput.innerHTML = htmlesc(call.output);
                } else {
    ...
    var headersList = function(headers) {
        var h = '';
        for (n in headers) h += '<li>'+htmlesc(n)+':&nbsp;'+htmlesc(headers[n]);
        return h;
    };
    ...
    document.getElementById('requestbody').innerHTML = htmlesc(request.body);
    ...
    document.getElementById('responsebody').innerHTML = htmlesc(response.body);
...
window.onload = function() {
    ...
    bg.src = '/lib/images/http-headers-status-v3.png';
};
