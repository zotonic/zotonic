...
%% Copyright (c) 2014, Maas-Maarten Zeeman <mmzeeman@xs4all.nl>
%%
...
%%
%% Lifted from cowboy and adapted to serve as websocket handler for zotonic
%%

%% Changes:
%%   * Removed ranch remove connection call.
%%   * Removed configurable transport => mochiweb_socket only
...
-record(state, {
	...
	socket = undefined, %% :: undefined | mochiweb socket(),
	handler :: module(),
...
websocket_extensions(State, Req) ->
	case z_ws_request_adapter:parse_header(<<"sec-websocket-extensions">>, Req) of
		{ok, Extensions, Req2} when Extensions =/= undefined ->
			[Compress] = z_ws_request_adapter:get([resp_compress], Req),
			case lists:keyfind(<<"x-webkit-deflate-frame">>, 1, Extensions) of
				{<<"x-webkit-deflate-frame">>, []} when Compress =:= true ->
					...
					}, z_ws_request_adapter:set_meta(websocket_compress, true, Req2)};
				_ ->
					{ok, State, z_ws_request_adapter:set_meta(websocket_compress, false, Req2)}
			end;
		_ ->
			{ok, State, z_ws_request_adapter:set_meta(websocket_compress, false, Req)}
	end.
...
handler_init(State=#state{env=Env, handler=Handler}, Req, HandlerOpts) ->
	try z_ws_request_adapter:websocket_handler_init(Req, Handler, HandlerOpts) of
...
websocket_handshake(State=#state{key=Key, deflate_frame=DeflateFrame, handler=Handler}, Req, HandlerState) ->
	...
	{ok, Req2} = z_ws_request_adapter:upgrade_reply(
		101,
		[{<<"Upgrade">>, <<"websocket">>},
...
handler_call(State=#state{handler=Handler}, Req, HandlerState,
	...
	try z_ws_request_adapter:websocket_handler_callback(Req, Handler, Callback, Message, HandlerState) of
...
websocket_send(Type, State=#state{socket=Socket})
	...
	case mochiweb_socket:send(Socket, << 1:1, 0:3, Opcode:4, 0:8 >>) of
		ok -> {shutdown, State};
		Error -> {Error, State}
	end;
websocket_send(Type, State=#state{socket=Socket})
	...
	Opcode = websocket_opcode(Type),
	{mochiweb_socket:send(Socket, << 1:1, 0:3, Opcode:4, 0:8 >>), State};
...
websocket_send({Type = close, StatusCode, Payload}, State=#state{
	...
	ok = mochiweb_socket:send(Socket,
...
websocket_close(State=#state{socket=Socket},
	...
	_OkOrError = case Reason of
		{normal, _} ->
			mochiweb_socket:send(Socket, << 1:1, 0:3, 8:4, 0:1, 2:7, 1000:16 >>);
		{error, badframe} ->
			mochiweb_socket:send(Socket, << 1:1, 0:3, 8:4, 0:1, 2:7, 1002:16 >>);
		{error, badencoding} ->
			mochiweb_socket:send(Socket, << 1:1, 0:3, 8:4, 0:1, 2:7, 1007:16 >>);
		% TODO: check why this can't be reached.
		{error, handler} ->
			mochiweb_socket:send(Socket, << 1:1, 0:3, 8:4, 0:1, 2:7, 1011:16 >>);
		{remote, closed} ->
		 	mochiweb_socket:send(Socket, << 1:1, 0:3, 8:4, 0:8 >>);
		{remote, Code, _} ->
...
handle_event(Req, Handler, Name, EventArgs, Opts) ->
	z_ws_request_adapter:websocket_handler_handle_event(Req, Handler, Name, EventArgs, Opts).
