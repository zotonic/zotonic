(function(win){var whiteSpaceRe=/^\s*|\s*$/g,undefined;var tinymce={majorVersion:'3',minorVersion:'3.2',releaseDate:'2010-03-25',_init:function(){var t=this,d=document,na=navigator,ua=na.userAgent,i,nl,n,base,p,v;t.isOpera=win.opera&&opera.buildNumber;t.isWebKit=/WebKit/.test(ua);t.isIE=!t.isWebKit&&!t.isOpera&&(/MSIE/gi).test(ua)&&(/Explorer/gi).test(na.appName);t.isIE6=t.isIE&&/MSIE [56]/.test(ua);t.isGecko=!t.isWebKit&&/Gecko/.test(ua);t.isMac=ua.indexOf('Mac')!=-1;t.isAir=/adobeair/i.test(ua);if(win.tinyMCEPreInit){t.suffix=tinyMCEPreInit.suffix;t.baseURL=tinyMCEPreInit.base;t.query=tinyMCEPreInit.query;return;}
...
return null;if(keep_children){while(child=node.firstChild){if(child.nodeType!==3||child.nodeValue)
...
return how==CLONE?n.cloneNode(TRUE):n;n.parentNode.removeChild(n);};};ns.Range=Range;})(tinymce.dom);(function(){function Selection(selection){var t=this,invisibleChar='\uFEFF',range,lastIERng,dom=selection.dom,TRUE=true,FALSE=false;function compareRanges(rng1,rng2){if(rng1&&rng2){if(rng1.item&&rng2.item&&rng1.item(0)===rng2.item(0))
return TRUE;if(rng1.isEqual&&rng2.isEqual&&rng2.isEqual(rng1)){try{range.startContainer.nextSibling;return TRUE;}catch(ex){}}}
return FALSE;};function getRange(){var ieRange=selection.getRng(),domRange=dom.createRng(),ieRange2,element,collapsed,isMerged;element=ieRange.item?ieRange.item(0):ieRange.parentElement();if(element.ownerDocument!=dom.doc)
...
ieRange2=ieRange.duplicate();collapsed=selection.isCollapsed();ieRange.collapse();ieRange.pasteHTML('<span id="_mce_start" style="display:none;line-height:0">'+invisibleChar+'</span>');if(!collapsed){ieRange2.collapse(FALSE);ieRange2.pasteHTML('<span id="_mce_end" style="display:none;line-height:0">'+invisibleChar+'</span>');}
...
r.pasteHTML(h);}
t.onSetContent.dispatch(t,s);},getStart:function(){var t=this,r=t.getRng(),e;if(r.duplicate||r.item){if(r.item)
...
return e.parentNode;return e;}},getEnd:function(){var t=this,r=t.getRng(),e,eo;if(r.duplicate||r.item){if(r.item)
...
point.push(offset);}else if(container.childNodes.length>0){childNodes=container.childNodes;if(offset>=childNodes.length){after=1;offset=childNodes.length-1;}
...
t.tridentSel.destroy();if(bookmark){if(bookmark.start){rng=dom.createRng();root=dom.getRoot();function setEndPoint(start){var point=bookmark[start?'start':'end'],i,node,offset;if(point){for(node=root,i=point.length-1;i>=1;i--)
node=node.childNodes[point[i]];if(start)
rng.setStart(node,point[0]);else
rng.setEnd(node,point[0]);}};setEndPoint(true);setEndPoint();t.setRng(rng);}else if(bookmark.id){rng=dom.createRng();function restoreEndPoint(suffix){var marker=dom.get(bookmark.id+'_'+suffix),node,idx,next,prev,keep=bookmark.keep;if(marker){node=marker.parentNode;if(suffix=='start'){if(!keep){idx=dom.nodeIndex(marker);}else{node=marker;idx=1;}
...
if(!r)
r=t.win.document.createRange?t.win.document.createRange():t.win.document.body.createTextRange();return r;},setRng:function(r){var s,t=this;if(!t.tridentSel){s=t.getSel();if(s){s.removeAllRanges();s.addRange(r);}}else{if(r.cloneRange){t.tridentSel.addRange(r);return;}
...
if(s.auto_focus){setTimeout(function(){var ed=tinymce.get(s.auto_focus);ed.selection.select(ed.getBody(),1);ed.selection.collapse(1);ed.getWin().focus();},100);}},1);e=null;},focus:function(sf){var oed,t=this,ce=t.settings.content_editable;if(!sf){if(!ce)
t.getWin().focus();}
...
ed.getWin().scrollTo(0,divYPos);};ed.onKeyPress.add(function(ed,e){if(e.keyCode==13&&(e.shiftKey||s.force_br_newlines)){insertBr(ed);Event.cancel(e);}});}
...
return;}}
return format;}}}};function match(name,vars,node){var startNode,i;function matchParents(node){node=dom.getParent(node,function(node){return!!matchNode(node,name,vars);});return matchNode(node,name,vars);};if(node)
...
if(hasPending()){ed.getDoc().execCommand('FontName',false,'mceinline');each(dom.select('font,span'),function(node){var bookmark;if(isCaretNode(node)){bookmark=selection.getBookmark();perform(node);selection.moveToBookmark(bookmark);ed.nodeChanged();}});if(!pendingFormats.isListening&&hasPending()){pendingFormats.isListening=true;each('onKeyDown,onKeyUp,onKeyPress,onMouseUp'.split(','),function(event){ed[event].addToTop(function(ed,e){if(hasPending()){each(dom.select('font,span'),function(node){var bookmark,textNode,rng;if(isCaretNode(node)){textNode=node.firstChild;perform(node);rng=dom.createRng();rng.setStart(textNode,textNode.nodeValue.length);rng.setEnd(textNode,textNode.nodeValue.length);selection.setRng(rng);ed.nodeChanged();}});if(e.type=='keyup'||e.type=='mouseup')
