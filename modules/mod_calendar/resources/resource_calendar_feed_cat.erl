...
%% @date 2009-12-06
%% @doc Serve an icalendar feed for a category.  Shows the newest created items of a certain category, always as the anonymous user.
...
content_types_provided(ReqData, Context) ->
    {[{"text/calendar", provide_content}], ReqData, Context}.
...
provide_content(ReqData, Context) ->
    ...
    F = fun() ->
        ...
        {Content, _Context2} = z_template:render_to_iolist("icalendar_feed_cat.tpl", Vars, Context1),
        Content
    end,
    Content = z_depcache:memo(F, {icalendar_feed, CatName}, ?MAX_AGE, [CatName], Context1),
