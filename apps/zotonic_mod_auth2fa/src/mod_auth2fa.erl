...
event(#postback{ message = {auth2fa_remove, Args} }, Context) ->
    ...
    case z_acl:is_allowed(use, mod_admin_identity, Context) orelse Id =:= UserId of
        true when Id =:= 1, UserId =/= 1 ->
            z_render:growl(?__("Only the admin can change the two-factor authentication of the admin.", Context),
                           Context);
        true ->
                ...
                false ->
                    z_render:wire({confirm,
                                   [{title, ?__("Add two-factor authentication", Context)},
                                    {text,
                                     ?__("You can add two-factor authentication to your account."
...
event(#postback{ message = {dialog_2fa, _Args} }, Context) ->
    case z_acl:user(Context) of
        ...
        UserId ->
            ...
            z_render:dialog(?__("Scan two-factor authentication passcode", Context),
                            "_dialog_auth2fa_passcode.tpl",
                            [{id, UserId}, {backdrop, static}, {request_key, RequestKey}],
                            Context)
    end.
...
observe_auth_postcheck(#auth_postcheck{
                           id = UserId,
