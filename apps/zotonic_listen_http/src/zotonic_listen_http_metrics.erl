...
metrics_callback(#{
    ...
    case ReqBodyLength of
        undefined ->
            ok;
        _ ->
            z_stats:record_count(http, data_in, ReqBodyLength, Site)
    end,
    case RespBodyLength of
        undefined ->
            ok;
        _ ->
            z_stats:record_count(http, data_out, RespBodyLength, Site)
    end,
...
queue('3xx') ->
    zotonic_http_metrics_normal;
queue('4xx') ->
    zotonic_http_metrics_prio;
queue('5xx') ->
    zotonic_http_metrics_prio.

http_status_category(switch_protocol, _) ->
    '1xx';
http_status_category(_, X) when X < 300 ->
    '2xx';
http_status_category(_, X) when X < 400 ->
    '3xx';
http_status_category(_, X) when X < 500 ->
    '4xx';
http_status_category(_, X) when X < 600 ->
    '5xx';
http_status_category(_, _X) ->
    xxx.
