...
{% if is_logon_link %}
    <div class="signup-logon-link">
        <p>{_ You already have an account. _}</p>
...
{% if is_code_sent %}
    <div class="signup-code-sent">
        ...
        <p>
            {% trans "We have sent a code to <strong>{email}</strong> to confirm your email address." email=email|escape %} {_ Please check your email and enter the code in the form below. _}
        </p>
        ...
        <form id="signup_form_step2" action="postback">
            <div class="form-group">
                <label for="signup-code-input" class="control-label">{_ Code _}</label>
...
{% if error and error != `exists` %}
    <div class="signup-error">
        {% if error == `email_send_failed` %}
            <p class="text-danger">
                {% trans "Could not send email to <strong>{email}</strong>. Please check the email address and try again."
                         email=email|escape
                %}
            </p>
        {% else %}
