...
observe_identity_verification(#identity_verification{
                                  user_id = UserId,
                                  identity = undefined
                              },
                              Context) ->
...
observe_logon_ready_page(#logon_ready_page{ request_page = None }, Context) when None =:= undefined; None =:= <<>> ->
    case z_auth:is_auth(Context) of
        true ->
            m_rsc:p(
                z_acl:user(Context), page_url, Context);
        false ->
            undefined
    end;
...
-spec signup_existing(integer() | undefined, map() | list(), list(), boolean(), z:context()) ->
...
request_verification(UserId, [Ident | Rest], Requested, Context) ->
    case z_notifier:first(#identity_verification{
                              user_id = UserId,
                              identity = Ident
                          },
                          Context)
    of
        ok ->
            request_verification(UserId, Rest, true, Context);
        _ ->
            request_verification(UserId, Rest, Requested, Context)
    end.
...
do_signup(UserId, Props, SignupProps, RequestConfirm, Context) ->
    ...
    case insert_or_update(UserId, props_to_rsc(Props, IsVerified, Context), Context) of
        {ok, NewUserId} ->
            ...
            ContextUser =
                case m_rsc:p_no_acl(NewUserId, creator_id, Context) of
                    NewUserId ->
                        z_acl:sudo(ContextLogon);
                    _ ->
                        ContextLogon
                end,
            ensure_identities(NewUserId, SignupProps, ContextUser),
            z_notifier:map(#signup_done{
                               id = NewUserId,
            ...
            case IsVerified of
                true ->
                    z_notifier:map(#signup_confirm{ id = NewUserId }, ContextUser);
                false ->
                    nop
            end,
...
maybe_add_depiction(Id, #{ <<"depiction_url">> := Url }, ContextUser)
    ...
    case z_convert:to_bool(
             m_config:get_value(mod_signup, depiction_as_medium, ContextUser))
    of
        false ->
            case m_edge:objects(Id, depiction, ContextUser) of
                [] ->
                    ...
                    case m_media:insert_url(Url, MediaProps, ContextUser) of
                        {ok, MediaId} ->
                            ?LOG_INFO("Added depiction from depiction_url for ~p: ~p", [Id, Url]),
                            {ok, _} = m_edge:insert(Id, depiction, MediaId, ContextUser),
                            ok;
                        {error, _} = Error ->
                            ?LOG_WARNING("Could not insert depiction_url for ~p: ~p ~p", [Id, Error, Url]),
                            Error
                    end;
                _ ->
                    ok
            ...
            case m_media:get(Id, ContextUser) of
                undefined ->
                    case m_media:replace_url(Url, Id, #{  }, ContextUser) of
                        {ok, _Id} ->
                            ?LOG_INFO("Added medium from depiction_url for ~p: ~p", [Id, Url]),
                            ok;
                        {error, _} = Error ->
                            ?LOG_WARNING("Could not insert depiction_url for ~p: ~p ~p", [Id, Error, Url]),
                            Error
...
has_verified_identity([{identity, {Type, _, _, true}} | _Is]) when Type /= username_pw ->
    true;
has_verified_identity([_ | Is]) ->
    has_verified_identity(Is).
...
props_to_rsc(Props, IsVerified, Context) when is_map(Props) ->
    Category =
        z_convert:to_atom(
            m_config:get_value(mod_signup, member_category, person, Context)),
    ContentGroup =
        z_convert:to_atom(
            m_config:get_value(mod_signup, content_group, undefined, Context)),
    Props1 = maps:without([<<"category">>, <<"content_group">>], Props),
    Props2 =
        Props1#{
    ...
    case maps:is_key(<<"title">>, Props2) of
        ...
        false ->
            ...
            Props2#{ <<"title">> => iolist_to_binary(Title) }
    end.
...
send_verify_email(UserId, Ident, Context) ->
    ...
    {ok, Key} =
        m_identity:set_verify_key(
            proplists:get_value(id, Ident), Context),
    Vars = [{recipient_id, UserId}, {user_id, UserId}, {email, Email}, {verify_key, Key}],
