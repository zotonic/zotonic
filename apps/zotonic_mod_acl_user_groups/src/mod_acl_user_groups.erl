...
is_acl_admin(Context) ->
    z_acl:is_allowed(use, mod_acl_user_groups, Context) andalso z_acl:is_allowed(insert, acl_user_group, Context).

event(#submit{ message = {delete_move, Args} }, Context) ->
    ToUGId =
        z_convert:to_integer(
            z_context:get_q_validated(<<"acl_user_group_id">>, Context)),
...
event(#postback{ message = {delete_all, Args} }, Context) ->
    ...
    case not IfEmpty orelse not m_acl_user_group:is_used(Id, Context) of
        ...
        false ->
            z_render:wire({alert, [{message, ?__("Delete is canceled, there are users in the user groups.", Context)}]},
                          Context)
    end.

%% @todo let the client subscribe to the resources to reflect the deletions
-spec ug_delete([m_rsc:resource_id()], z:context()) -> any().
ug_delete(Ids, Context) ->
    ...
    case unlink_all(UGUserIds, 0, Total, Context) of
        ...
        {error, _} ->
            Actions = [{unmask, []}, {alert, [{message, ?__("Not all user groups could be deleted.", Context)}]}],
...
observe_acl_is_owner(#acl_is_owner{
                         id = Id,
                         user_id = Id
                     },
                     _Context) ->
...
observe_acl_is_allowed_prop(#acl_is_allowed_prop{
                                action = view,
                                object = undefined
                            },
                            _Context) ->
...
observe_hierarchy_updated(#hierarchy_updated{
                              root_id = <<"$category">>,
                              predicate = undefined
                          },
                          Context) ->
    rebuild(Context);
observe_hierarchy_updated(#hierarchy_updated{
                              root_id = <<"content_group">>,
                              predicate = undefined
                          },
                          Context) ->
    rebuild(Context);
observe_hierarchy_updated(#hierarchy_updated{
                              root_id = <<"acl_user_group">>,
                              predicate = undefined
                          },
                          Context) ->
    rebuild(Context);
...
observe_rsc_insert(#rsc_insert{ props = RscProps }, InsertProps, Context) ->
    case maps:get(<<"content_group_id">>, RscProps, maps:get(<<"content_group_id">>, InsertProps, undefined)) of
        undefined ->
...
observe_rsc_update(#rsc_update{
    ...
    case maps:is_key(<<"acl_mime_allowed">>, NewProps1) orelse maps:is_key(<<"acl_upload_size">>, NewProps1) of
        true ->
...
observe_rsc_update_done(#rsc_update_done{
                        ...
                        Context) ->
    case lists:member(acl_user_group, PreIsA) orelse lists:member(acl_user_group, PostIsA) of
        true ->
        ...
        true ->
            case m_acl_user_group:is_used(Id, Context) of
                true ->
                    throw({error, is_used});
                false ->
                    z:info("Deleting user group ~p ('~s')", [Id, title_bin(Id, Context)], [{module, ?MODULE}], Context),
                    ok
            end;
        false ->
...
observe_edge_insert(#edge_insert{
                        subject_id = UserId,
                        predicate = hasusergroup
                    } =
                        L,
                    Context) ->
...
observe_edge_delete(#edge_delete{
                        subject_id = UserId,
                        predicate = hasusergroup
                    } =
                        L,
...
log_membership(#edge_insert{
               ...
               Context) ->
    z:info("User ~p (~s) added to user group ~p ('~s')",
           [UserId, email_bin(UserId, Context), UGId, title_bin(UGId, Context)],
           [{module, ?MODULE}, {user_id, edge_user(EdgeId, Context)}],
           Context);
log_membership(#edge_delete{
                   predicate = hasusergroup,
                   subject_id = UserId,
                   object_id = UGId
               },
               Context) ->
    z:info("User ~p (~s) removed from user group ~p ('~s')",
           [UserId, email_bin(UserId, Context), UGId, title_bin(UGId, Context)],
...
log_membership(#edge_insert{
               ...
               Context) ->
    z:info("User ~p (~s) added to collaboration group ~p ('~s')",
           [UserId, email_bin(UserId, Context), UGId, title_bin(UGId, Context)],
           [{module, ?MODULE}, {user_id, edge_user(EdgeId, Context)}],
           Context);
log_membership(#edge_delete{
               ...
               Context) ->
    z:info("User ~p (~s) removed from collaboration group ~p ('~s')",
           [UserId, email_bin(UserId, Context), UGId, title_bin(UGId, Context)],
           [{module, ?MODULE}, {user_id, undefined}],
           Context);
log_membership(#edge_insert{
               ...
               Context) ->
    z:info("User ~p (~s) added as manager to collaboration group ~p ('~s')",
           [UserId, email_bin(UserId, Context), UGId, title_bin(UGId, Context)],
           [{module, ?MODULE}, {user_id, edge_user(EdgeId, Context)}],
           Context);
log_membership(#edge_delete{
               ...
               Context) ->
    z:info("User ~p (~s) removed as manager from collaboration group ~p ('~s')",
           [UserId, email_bin(UserId, Context), UGId, title_bin(UGId, Context)],
           [{module, ?MODULE}, {user_id, undefined}],
           Context).
...
edge_user(EdgeId, Context) ->
    case m_edge:get(EdgeId, Context) of
        undefined ->
            undefined;
        Edge ->
            proplists:get_value(creator_id, Edge)
    end.
    ...
    z_html:unescape(
        z_convert:to_binary(
            z_trans:lookup_fallback(
                m_rsc:p_no_acl(Id, title, Context), Context))).
...
signal_user_changed(_UserId, _Context) ->
    ok.    % Sessions = z_session_manager:list_sessions_user(UserId, Context),
           % lists:foreach(
           %     fun(Session) ->
...
handle_info(rebuild, #state{ rebuilder_pid = Pid } = State) when is_pid(Pid) ->
    {noreply, State};
handle_info({'DOWN', MRef, process, _Pid, normal},
            #state{
                rebuilder_mref = MRef,
                site = Site
            } =
                State) ->
    ?LOG_DEBUG("[mod_acl_user_groups] rebuilder for ~p finished.", [State#state.rebuilding]),
    State1 =
        State#state{
    ...
    z_mqtt:publish(<<"model/acl_user_groups/event/acl-rules/",
                     (z_convert:to_binary(State#state.rebuilding))/binary,
                     "-rebuild">>,
                   true,
                   z_acl:sudo(Context)),
    {noreply, State2};
handle_info({'DOWN', MRef, process, _Pid, Reason}, #state{ rebuilder_mref = MRef } = State) ->
    ?LOG_ERROR("[mod_acl_user_groups] rebuilder for ~p down with reason ~p", [State#state.rebuilding, Reason]),
    State1 =
        case State#state.rebuilding of
            publish ->
                State#state{ is_rebuild_publish = true };
            edit ->
                State#state{ is_rebuild_edit = true }
        end,
...
manage_schema(Version, Context) ->
    ...
    #datamodel{
        categories =
            [{acl_user_group, meta, [{title, {trans, [{en, <<"User Group">>}, {nl, <<"Gebruikersgroep">>}]}}]},
        ...
        resources =
            [{acl_user_group_anonymous,
             ...
             {acl_user_group_editors, acl_user_group, [{title, {trans, [{en, <<"Editors">>}, {nl, <<"Redactie">>}]}}]},
        ...
        [] ->
            ...
            NewTree =
                [{R(acl_user_group_anonymous),
                  [{R(acl_user_group_members), [{R(acl_user_group_editors), [{R(acl_user_group_managers), []}]}]}]}],
