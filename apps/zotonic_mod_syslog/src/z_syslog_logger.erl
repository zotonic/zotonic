...
fmt(Site, DispatchRule, Time, Ip, User, Method, Path, Version, Status, Length, Referrer, UserAgent) ->
    iolist_to_binary([Site, $:, DispatchRule, $\s, Ip, $\s, z_convert:to_binary(User), $\s, Time, $\s, $", Method, $\s,
                      Path, $\s, Version, $", $\s, Status, $\s, Length, $\s, $", Referrer, $", $\s, $", UserAgent, $"]).
