...
terminal_notifier({unix, darwin}, Msg) ->
    exec:run(
        "which terminal-notifier && terminal-notifier -group zotonic "
        "-appIcon 'https://zotonic.com/apple-touch-icon.png' "
        "-contentImage 'https://zotonic.com/apple-touch-icon.png' "
        "-title Zotonic "
        "-message " ++ z_filelib:os_escape(Msg), []);
terminal_notifier({unix, _Arch}, Msg) ->
    exec:run(
        "which notify-send && notify-send -t 2000 \"Zotonic\" "
