...
With MQTT messages are published to topics. All subscribers to a topic will then receive the message. A topic is a
string, much like a file-path, for example: `truck/0001/temperature`

A subscriber can directly subscribe to a topic, or can use wildcards to subscribe to related topics. For this the
wildcards `+` and `#` can be used. `+` matches exactly one word between the slashes, and `#` can be used at the end of a
pattern to match all sub-topics.
...
A publisher can publish a *retained* message to a topic. When publishing all current topic subscribers will receive the
message. Above that, if a new subscription is made to the topic, then all retained messages are sent to the new subscriber.



### Quality of service

MQTT has three levels for the quality of message delivery. These are used when sending messages between machines. The
levels are:
...
A client can set a *last will* message and topic. This is a message that will be published to the topic at the moment
the client is unexpectedly disconnected.
...
Each open HTML page can also have a local (simplified) broker. The system can relay messages between the brokers on open
pages and the central broker in Zotonic. In this way it is possible for HTML pages to have their own local
publish/subscribe system and also subscribe or publish to topics on the central broker.

As the central broker is shared between sites it is even possible to publish/subscribe between different sites. In the
future it will be possible to bridge the brokers between servers.
...
The *Context* is the context of the process/user that subscribed to the message. Use the `publisher_context` for the
*Context* (and ACL permissions) of the publisher.
...
This will subscribe the function, with the current process (`self()`) as the managing process. If the process exits then
the subscription is removed.

Access control applies and the result `{error, eacces}` will be returned if access is denied, `ok` will be returned on a
...
When the process stops it will automatically be unsubscribed. The process will receive messages `{mqtt_msg, map()}`,
where the `map()` is like the map in the section above.
...
All connections must authenticate using a username and password. The username is prefixed with the hostname of the
...
- `observe_action_event_type`: Register the `{mqtt, ...}` action event type so wire actions can subscribe through MQTT.
- `observe_module_activate`: On module activation, scan the module for exported `mqtt:` observer callbacks and subscribe them as MQTT handlers using a sudo context.
