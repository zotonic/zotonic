...
The HTTP POSTs must have a valid ticket. See `model#mqtt_ticket` on how to obtain such
...
*   Cookie authentication with `z.auth` cookie, see `controller#controller_authentication`.
