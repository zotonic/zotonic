...
event(#submit{ message = {delete_move, Args} }, Context) ->
    ToCatId =
        z_convert:to_integer(
            z_context:get_q_validated(<<"category_id">>, Context)),
...
event(#postback{ message = {delete_all, Args} }, Context) ->
    ...
    case not IfEmpty orelse not m_category:is_used(Id, Context) of
        ...
        false ->
            z_render:wire({alert, [{text, ?__("Delete is canceled, there are pages in the category.", Context)}]},
                          Context)
    end.
...
cat_delete(Ids, Context) ->
    ...
    case delete_all(RscIds, 0, length(RscIds), Context) of
        ...
        {error, _} ->
            Actions = [{unmask, []}, {alert, [{text, ?__("Not all resources could be deleted.", Context)}]}],
...
delete_all([Id | Ids], N, Total, Context) ->
    case catch m_rsc:delete(Id, Context) of
        ...
        Error ->
            ?LOG_ERROR("mod_admin_category: could not delete ~p: ~p", [Id, Error]),
            {error, Error}
    end.
...
maybe_progress(N1, N2, Total, Context) ->
    ...
    case S1 of
        S2 ->
            ok;
        _ ->
            z_notifier:first(#page_actions{ actions = [{mask_progress, [{percent, S2}]}] }, Context)
    end.
