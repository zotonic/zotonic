...
-record(state,
        {site :: atom() | undefined, admin_log_pages = [] :: list(), dedup :: map(), last_ui_event = 0 :: integer()}).
...
observe_search_query(#search_query{
    ...
    case z_acl:is_allowed(use, mod_logging, Context) of
        true ->
            m_log:search_query(Args, Context);
        false ->
            []
    end;
observe_search_query(#search_query{
    ...
    case z_acl:is_allowed(use, mod_logging, Context) of
        true ->
            m_log_email:search(Args, Context);
        false ->
            []
    end;
observe_search_query(#search_query{
    ...
    case z_acl:is_allowed(use, mod_logging, Context) of
        true ->
            m_log_ui:search_query(Args, Context);
        false ->
            []
    end;
...
pid_observe_zlog(Pid,
                 #zlog{
                     user_id = LogUser,
                     props = #log_message{ props = MsgProps } = Msg
                 },
                 Context) ->
...
is_ui_ratelimit_check(Context) ->
    case z_convert:to_bool(
             m_config:get_value(mod_logging, ui_log_disabled, Context))
    of
        true ->
            false;
        false ->
...
handle_cast({log, OtherLog}, State) ->
    ...
    {noreply, State};
handle_cast(check_db_pool_health,
            #state{
                site = Site,
                dedup = Dedup
            } =
...
handle_simple_log(#log_message{
    ...
    Message = [{user_id, UserId}, {type, Type}, {message, Msg}] ++ proplists:delete(user_id, Props),
    ...
    LogTypeB =
        z_convert:to_binary(
            proplists:get_value(log_type, Props, log)),
    z_mqtt:publish([<<"model">>, <<"logging">>, <<"event">>, LogTypeB, SeverityB],
                   #{
                       log_id => Id,
                       user_id => UserId
                   },
    ...
    case z_db:table_exists(LogType, Context) of
        ...
        false ->
            Log = #log_message{
                      message =
                          z_convert:to_binary(
                              proplists:get_value(message, Fields, LogType)),
...
flatten_prop({K, V} = Prop) ->
    case is_date(V) of
        true ->
            Prop;
        false ->
            {K, z_convert:to_binary(V)}
    end.

is_date({{_, _, _}, {_, _, _}}) ->
    true;
is_date({Y, M, D}) when is_integer(Y); is_integer(M); is_integer(D) ->
    true;
is_date(_) ->
    false.
...
record_to_log_message(#log_email{  } = R, _Fields, LogType, Id) ->
    #log_message{
        message =
            iolist_to_binary(["SMTP: ",
                              z_convert:to_list(R#log_email.mailer_status),
                              ": ",
                              to_list(R#log_email.mailer_message),
                              $\n,
                              "To: ",
                              z_convert:to_list(R#log_email.envelop_to),
                              opt_user(R#log_email.to_id),
                              $\n,
                              "From: ",
                              z_convert:to_list(R#log_email.envelop_from),
                              opt_user(R#log_email.from_id)]),
