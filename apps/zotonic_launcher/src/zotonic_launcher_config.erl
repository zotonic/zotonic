...
maybe_initialize_configs(Dir) ->
    SourceDir =
        case code:priv_dir(zotonic_launcher) of
            {error, bad_name} ->
                Beam = code:where_is_file("zotonic_launcher_config.beam"),
                Top = filename:dirname(
    ...
    case filelib:is_file(
             filename:join([Dir, "erlang.config"]))
    of
        false ->
    ...
    case filelib:is_file(
             filename:join([Dir, "zotonic.config"]))
    of
        false ->
...
zotonic_config_files(Node) ->
    case config_dir(Node) of
        {ok, Dir} ->
            Files =
                lists:filter(fun(F) ->
                                not is_erlang_config(F)
                             end,
                             z_config_files:files(Dir)),
            SubFiles =
                z_config_files:files(
                    filename:join([Dir, "config.d"])),
            Files ++ SubFiles;
        {error, _} ->
            []
    end.
...
-spec erlang_config_files(node()) -> [file:filename()].
...
-spec config_files(node()) -> [file:filename()].
config_files(Node) ->
    case config_dir(Node) of
        {ok, Dir} ->
            Files = z_config_files:files(Dir),
...
apps_config(File, Data, Cfgs) when is_list(Data) ->
    lists:foldl(fun (AppConfig, Acc) when is_map(AppConfig) ->
                        % Map:  #{ App => AppConfig }
                        maps:fold(fun (App, Cfg, {ok, MAcc}) ->
                    ...
                    (AppConfig, Acc) when is_list(AppConfig) ->
                        ...
                        lists:foldl(fun ({App, Cfg}, {ok, MAcc}) ->
                                            app_config(App, Cfg, MAcc);
                                        (Other, {ok, _}) ->
