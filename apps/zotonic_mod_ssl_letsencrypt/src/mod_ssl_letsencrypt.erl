...
observe_tick_24h(tick_24h, Context) ->
    load_cert(Context),
    gen_server:cast(
        z_utils:name_for_site(?MODULE, Context), renewal_check).
    ...
    case z_acl:is_admin_editable(Context) of
        true ->
            ...
            case gen_server:call(
                     z_utils:name_for_site(?MODULE, Context), {cert_request, Hostname, SANs1})
            of
                ok ->
                    z_render:update(Wrapper,
                                    #render{
                                        template = "_admin_ssl_letsencrypt_running.tpl",
                                        vars = [{hostname, Hostname}, {san, SANs1}]
                                    },
                                    Context);
                {error, Reason} ->
                    ...
                    z_render:wire({alert,
                                   [{title, ?__(<<"SSL Let’s Encrypt Certificate"/utf8>>, Context)},
                                    {text,
                                     ?__("Could not start fetching the SSL certificate. Try again later.", Context)},
...
get_self_ping(Context) ->
    gen_server:call(
        z_utils:name_for_site(?MODULE, Context), get_self_ping).
...
handle_call({cert_request, Hostname, SANs}, _From, State) ->
    case start_cert_request(Hostname, SANs, State) of
        {ok, State1} ->
            z_mqtt:publish(<<"model/letsencrypt/event/status">>,
                           <<"started">>,
                           z_acl:sudo(
                               z_context:new(State#state.site))),
            {reply, ok, State1};
        {error, Reason, State1} ->
...
handle_cast(renewal_check,
    ...
    case NextMonth > State#state.cert_valid_till andalso Now < State#state.cert_valid_till of
        true ->
    ...
    case filelib:is_file(CertFile) of
        true ->
            case zotonic_ssl_certs:decode_cert(CertFile) of
                ...
                {error, _} = Error ->
                    ?LOG_ERROR("Could not decode Letsencrypt crt file ~p", [Error]),
...
start_cert_request(Hostname,
    ...
    LetsOpts = [{cert_path, CertPath}, {key_file, KeyFile} | ?ACME_SRV_OPTS],
        ...
        lists:filtermap(fun(D) ->
                           case z_string:trim(D) of
                               <<>> ->
                                   false;
                               D1 ->
                                   {true, <<D1/binary, 10, "-----END CERTIFICATE-----", 10>>}
                           end
                        end,
                        Parts),
    case Parts1 of
...
cert_files(Context) ->
    ...
    Files =
        [{certfile,
          z_convert:to_list(
              filename:join(SSLDir, <<Hostname/binary, ".crt">>))},
         {keyfile,
          z_convert:to_list(
              filename:join(SSLDir, <<Hostname/binary, ".key">>))}]
    ...
    case filelib:is_file(CaCertFile) of
        false ->
            {ok, Files};
        true ->
            {ok, [{cacertfile, CaCertFile} | Files]}
    end.
...
cert_files_all(Context) ->
    ...
    {ok,
     [{certfile,
      ...
      {cacertfile,
       z_convert:to_list(
           filename:join(SSLDir, <<Hostname/binary, ".ca.crt">>))},
      {keyfile,
       z_convert:to_list(
           filename:join(SSLDir, <<Hostname/binary, ".key">>))}]}.
...
-spec check_keyfile(string(), z:context()) ->
                       ok | {error, openssl | no_private_keys_found | need_rsa_private_key | term()}.
check_keyfile(KeyFile, Context) ->
    ...
    case file:read_file(KeyFile) of
        ...
        {error, _} = Error ->
            ?LOG_ERROR("[~p] Cannot read Letsencrypt key file ~p, error: ~p", [Site, KeyFile, Error]),
            Error
    end.

%% @doc Ensure that we have a RSA key for Letsencrypt.
-spec ensure_key_file(z:context()) ->
...
ensure_key_file(Context) ->
    ...
    case filelib:is_file(KeyFile) of
        ...
        false ->
            ...
            _ = file:change_mode(
                    filename:basename(KeyFile), 8#00700),
            Escaped = z_filelib:os_filename(KeyFile),
            Cmd = "openssl genrsa -out " ++ Escaped ++ " " ++ z_convert:to_list(?KEY_BITS),
            Result = os:cmd(Cmd),
            case filelib:is_file(KeyFile) of
                true ->
                    _ = file:change_mode(KeyFile, 8#00600),
                ...
                false ->
                    ?LOG_ERROR("Error generating RSA key for LetsEncrypt: ~p", [Result]),
                    {error, openssl}
            end
    end.
        ...
        {ok, {_Url, Hs, _Size, Cert}} ->
            case proplists:get_value("content-type", Hs) of
                ...
                "application/x-pem-file" ->
                    save_ca_cert(Cert, Context);
                CT ->
                    ?LOG_ERROR("Download of ~p returned a content-type ~p", [?CA_CERT_URL, CT]),
                    {error, content_type}
            end;
        {error, _} = Error ->
            ?LOG_ERROR("Download of ~p error ~p", [?CA_CERT_URL, Error]),
            Error
    end.
