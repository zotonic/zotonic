...
event(#postback{ message = vcs_up }, Context) ->
    ...
    case m_site_update:vcs_site(Site) of
        {_, _} = VCS ->
            z_notifier:notify(#page_actions{
                                  actions =
                                      [{growl, [{text, ?__("Fetching updates from the version control...", Context)}]}]
                              },
                              Context),
            ...
            case proplists:get_value(zotonic, Args) of
                true ->
                    case m_site_update:vcs_zotonic() of
                        ...
                        false ->
                            render_notice('Zotonic',
                                          ?__("Zotonic has not been checked out using version control.", Context),
                                          Context)
                    end;
                undefined ->
                    ...
                    case m_site_update:vcs_site(Site) of
                        ...
                        false ->
                            render_notice(Site,
                                          ?__("Unknown site or no version control folder present.", Context),
                                          Context)
                    end
            end;
        _ ->
...
async_notice(Sitename, Text, Context) ->
    z_notifier:notify(#page_actions{ actions = notice(Sitename, Text) }, Context).
