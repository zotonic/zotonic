...
maybe_load_edit_panel(Args, Context) ->
    case m_rsc:rid(
             z_context:get_q(<<"id">>, Context, proplists:get_value(id, Args)), Context)
    of
        undefined ->
        ...
        Id ->
            Vars =
                [{id, Id},
                 {tree_id,
                  m_rsc:rid(
                      z_context:get_q(<<"tree_id">>, Context), Context)}],
            Context1 =
                z_render:update("editcol",
                                #render{
                                    template = {cat, "_admin_frontend_edit.tpl"},
                                    vars = Vars
                                },
                                Context),
...
maybe_load_edit_cat(Args, Context) ->
    case m_rsc:rid(
             z_context:get_q(<<"cat">>, Context, proplists:get_value(cat, Args)), Context)
    of
        undefined ->
            Context;
        CatId ->
