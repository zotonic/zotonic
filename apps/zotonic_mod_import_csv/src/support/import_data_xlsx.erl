...
%% @doc Import a xlsx file according to the derived file/record definitions.
%% @end

%% Copyright 2025 Marc Worrell, Arjan Scherpenisse
%%
...
-spec import(File, DataDef, IsReset, Context) -> import_rows_data:importresults() when
    ...
    IsReset :: boolean(),
    Context :: z:context().
import(File, #import_data_def{ importstate = Rows } = Def, IsReset, Context) when is_list(Rows) ->
