...
-spec observe(zotonic_notifier:notifier(), zotonic_notifier:event(), zotonic_notifier:observer(), pid(), integer()) ->
                 ok | {error, term()}.
...
first_1([Obs | Rest], Msg, ContextArg) ->
    case notify_observer(Msg, Obs, true, ContextArg) of
        undefined ->
            first_1(Rest, Msg, ContextArg);
        continue ->
            first_1(Rest, Msg, ContextArg);
        {continue, Msg1} ->
            first_1(Rest, Msg1, ContextArg);
        Result ->
            Result
    end.
...
await_receive(Msg, Timeout) when is_atom(Msg) ->
    receive
        Msg ->
            {ok, Msg};
        M when is_tuple(M), element(1, M) =:= Msg ->
            {ok, M};
        {'$gen_cast', Msg} ->
            {ok, Msg};
        {'$gen_cast', M} when is_tuple(M), element(1, M) =:= Msg ->
            {ok, M};
        {'$gen_call', From, Msg} ->
            {ok, From, Msg};
        {'$gen_call', From, M} when is_tuple(M), element(1, M) =:= Msg ->
            {ok, From, M}
    after Timeout ->
        {error, timeout}
    end.
...
do_detach_observer(Table, Event, OwnerPid) ->
    case ets:lookup(Table, Event) of
        ...
        [{Event, Observers}] ->
            UpdatedObservers =
                lists:filter(fun({_Prio, _Obs, Pid}) ->
                                Pid =/= OwnerPid
                             end,
                             Observers),
...
notify_observer(Msg, {_Prio, Pid, _OwnerPid}, true, ContextArg) when is_pid(Pid) ->
    try
        ...
        EM:E:Trace ->
            ?LOG_ERROR("Error notifying ~p with event ~p. Error ~p:~p", [Pid, Msg, EM, E], #{ stack => Trace }),
...
notify_observer_fold(Msg, {_Prio, Pid, _OwnerPid}, Acc, ContextArg) when is_pid(Pid) ->
    try
        ...
        EM:E:Trace ->
            ?LOG_ERROR("Error folding ~p with event ~p. Error ~p:~p", [Pid, Msg, EM, E], #{ stack => Trace }),
            Acc
    end;
