...
-spec spam_check(binary()) ->
                    {ok, {ham | spam, SpamdStatus :: [{binary(), binary()}], SpamdHeaders :: [{binary(), binary()}]}} |
                    {error, term()}.
    ...
    case gen_tcp:connect(SpamDServer, SpamDPort, [binary]) of
        {ok, Socket} ->
            gen_tcp:send(Socket,
                         [<<"HEADERS SPAMC/1.2\r\n">>,
                          <<"Content-length: ">>,
                          integer_to_list(size(EncodedMail) + 2),
                          <<"\r\n">>,
                          <<"User: spamd\r\n">>,
                          <<"\r\n">>,
                          EncodedMail,
...
spam_status(<<"Yes,", RestStatus/binary>>) ->
    {true, parse_status_args(RestStatus)};
spam_status(<<"No,", RestStatus/binary>>) ->
    {false, parse_status_args(RestStatus)}.
...
is_spamd_header({<<"X-Spam", _/binary>>, _}) ->
    true;
is_spamd_header(_) ->
    false.
...
test() ->
     ...
     [{<<"score">>, <<"-0.11">>},
      ...
      {<<"tests">>,
       [[<<"AWL">>, <<"0.159">>],
        ...
        [<<"T_REMOTE_IMAGE">>, <<"0.01">>]]},
      {<<"autolearn">>, <<"no">>}]} =
        spam_status(<<"No, score=-0.11 tagged_above=-10 required=4.9 tests=[AWL=0.159,\r\n"
    ...
    {true,
     [{<<"score">>, <<"1001.1">>},
      ...
      {<<"version">>, <<"3.4.0'">>}]} =
        spam_status(<<"Yes, score=1001.1 required=5.0 tests=ALL_TRUSTED,\r\n"
