...
received(Recipients, From, Peer, Reference, {Type, Subtype}, Headers, Params, Body, Data) ->
    ...
    lists:map(fun(Recipient) ->
                 received(Recipient, ParsedEmail2, From, Peer, Reference, {Type, Subtype}, Headers, Params, Body, Data)
              end,
              Recipients).

received(Recipient, ParsedEmail, From, Peer, Reference, {Type, Subtype}, Headers, Params, Body, Data) ->
        ...
        {ok, {LocalPart, LocalTags, Domain, Site}} ->
            ...
            case z_notifier:first(Email1, Context) of
                {ok, MsgId} when is_binary(MsgId) ->
                    ?LOG_NOTICE("SMTP received email for <~s>, handled as ~p", [Recipient, MsgId]),
                    {ok, MsgId};
                {ok, Other} ->
                    ?LOG_NOTICE("SMTP received email for <~s>, handled as ~p", [Recipient, Other]),
                    {ok, undefined};
                ok ->
                    ?LOG_NOTICE("SMTP received email for <~s>, handled as undefined", [Recipient]),
                    {ok, undefined};
                {error, Reason} = Error ->
                    ?LOG_WARNING("SMTP received email for <~s>, handler returned error ~p", [Recipient, Reason]),
                    Error;
                undefined ->
                    ?LOG_WARNING("SMTP received email for <~s>, unhandled assuming unknown recipient", [Recipient]),
                    {error, unknown_recipient};
                Other ->
                    ?LOG_WARNING("SMTP received email for <~s>, unexpected return value ~p", [Recipient, Other]),
                    {ok, Other}
            end;
        {error, unknown_host} ->
...
-spec get_site(binary()) -> {ok, {binary(), [binary()], binary(), atom()}} | {error, unknown_host | not_running}.
get_site(Recipient) ->
    [Username, Domain] = binstr:split(Recipient, <<"@">>, 2),
    [LocalPart | LocalTags] = binstr:split(Username, <<"+">>),
    case z_sites_dispatcher:get_site_for_hostname(
             z_string:to_lower(Domain))
    of
        {ok, Site} ->
...
parse_email({<<"text">>, <<"html">>}, _Headers, Params, Body) ->
    case opt_attachment({<<"text">>, <<"html">>}, Params, Body) of
        #email{  } = E ->
            E;
        undefined ->
            #email{ html = z_html:sanitize(Body) }
    end;
...
parse_email({<<"message">>, <<"rfc822">>}, _Headers, _Params, Body) ->
    ...
    Text =
        iolist_to_binary([<<"From: ">>,
                          proplists:get_value(<<"From">>, FwdHeaders, <<>>),
                          $\n,
                          <<"To: ">>,
                          proplists:get_value(<<"To">>, FwdHeaders, <<>>),
                          $\n,
                          case Cc of
                              undefined ->
                                  <<>>;
                          ...
                          <<"Subject: ">>,
                          proplists:get_value(<<"Subject">>, FwdHeaders, <<>>),
                          $\n,
                          <<"Date: ">>,
                          proplists:get_value(<<"Date">>, FwdHeaders, <<>>),
                          $\n,
                          $\n]),
    Html =
        iolist_to_binary([<<"<dl class='rfc822-header'>">>,
                          <<"<dt>From:</dt><dd>">>,
                          z_html:escape(
                              proplists:get_value(<<"From">>, FwdHeaders, <<>>)),
                          <<"</dd>">>,
                          ...
                          case Cc of
                              undefined ->
                                  <<>>;
                              _ ->
                                  [<<"<dt>Cc:</dt><dd>">>, z_html:escape(Cc), <<"</dd>">>]
                          end,
                          <<"<dt>Subject:</dt><dd>">>,
                          z_html:escape(
                              proplists:get_value(<<"Subject">>, FwdHeaders, <<>>)),
...
append_mime(undefined, B) ->
    B;
append_mime(A, undefined) ->
    A;
append_mime(A, B) ->
    <<A/binary, $/, B/binary>>.
