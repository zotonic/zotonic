...
is_auto_from(undefined) ->
    false;
is_auto_from(EnvelopFrom) ->
    is_auto_from_1(z_convert:to_binary(
                       z_string:to_lower(EnvelopFrom))).

is_auto_from_1(<<"mailer-daemon@", _/binary>>) ->
    true;
is_auto_from_1(_) ->
    false.

is_auto_subject(undefined) ->
    false;
is_auto_subject(Subject) ->
    is_auto_subject_1(z_convert:to_binary(
                          z_string:to_lower(Subject))).

is_auto_subject_1(<<"auto:"/utf8, _/binary>>) ->
    true;
is_auto_subject_1(<<"automatic reply"/utf8>>) ->
    true;
is_auto_subject_1(<<"autosvar"/utf8>>) ->
    true;
is_auto_subject_1(<<"automatisk svar"/utf8>>) ->
    true;
is_auto_subject_1(<<"automatisch antwoord"/utf8>>) ->
    true;
is_auto_subject_1(<<"abwesenheitsnotiz"/utf8>>) ->
    true;
is_auto_subject_1(<<"risposta non al computer"/utf8>>) ->
    true;
is_auto_subject_1(<<"auto response"/utf8>>) ->
    true;
is_auto_subject_1(<<"eespuesta automática"/utf8>>) ->
    true;
is_auto_subject_1(<<"fuori sede"/utf8>>) ->
    true;
is_auto_subject_1(<<"out of office"/utf8>>) ->
    true;
is_auto_subject_1(<<"frånvaro"/utf8>>) ->
    true;
is_auto_subject_1(<<"réponse automatique"/utf8>>) ->
    true;
is_auto_subject_1(_) ->
    false.
        ...
        {_, Subject} ->
            Subject1 =
                z_convert:to_binary(
                    z_string:to_lower(Subject)),
            case Subject1 of
                <<"delivery notification", _/binary>> ->
                    true;
                <<"delivery status notification", _/binary>> ->
                    true;
                <<"mail delivery failed", _/binary>> ->
                    true;
                <<"undelivered mail", _/binary>> ->
                    true;
                <<"undeliverable:", _/binary>> ->
                    true;
                <<"returned mail", _/binary>> ->
                    true;
                <<"mail system error", _/binary>> ->
                    true;
                _ ->
                    false
            end
...
is_nonfatal_bounce(Type, Headers, Parts) ->
    is_nonfatal_subject(Headers) orelse is_nonfatal_status(Type, Parts).
    ...
    case lists:keyfind(<<"Subject">>, 1, Headers) of
        ...
        {_, Subject} ->
            Subject1 =
                z_convert:to_binary(
                    z_string:to_lower(Subject)),
            contains_1(Subject1, <<"delay">>) orelse contains_1(Subject1, <<"warning">>)
    end.
...
is_nonfatal_status({<<"multipart">>, <<"report">>}, Parts) ->
    Status = [Body || {<<"message">>, <<"delivery-status">>, _Hs, _Disp, Body} <- Parts],
    case Status of
        [Body | _] ->
            Body1 = binary:replace(Body, <<"\r\n\r\n">>, <<"\r\n">>, [global]),
            Hs = mochiweb_headers:to_list(
                     mochiweb_headers:from_binary(<<Body1/binary, "\r\n\r\n">>)),
            is_action_delayed(Hs) orelse is_nonfatal_diagnostic(Hs) orelse is_nonfatal_code(Hs);
        [] ->
            false
    end;
...
get_diagnostic(Hs) ->
    case z_convert:to_binary(
             proplists:get_value("Diagnostic-Code", Hs))
    of
        <<"smtp;  ", C/binary>> ->
            C;
        <<"smtp; ", C/binary>> ->
