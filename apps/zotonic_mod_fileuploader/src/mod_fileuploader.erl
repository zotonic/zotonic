...
event(#z_msg_v1{ data = Data }, Context) ->
    ...
    Context1 =
        lists:foldl(fun(#{ <<"upload">> := Upload }, Ctx) ->
...
do_upload({ok, #{ is_complete := true } = Status}, Data, Context) ->
    ...
    Context1 =
        case m_media:insert_file(TmpFile, RscProps1, [], Context) of
            {ok, RscId} ->
...
maybe_get(K, M, D) when is_map(M) ->
    maps:get(K, M, D);
maybe_get(_, _, D) ->
    D.

rsc_props_from_data(RscProps, #{ <<"subject_id">> := Id }, Context) ->
    RscProps#{ <<"content_group_id">> => m_rsc:p(Id, content_group_id, Context) };
...
-spec start_child(Filename :: binary(), Size :: non_neg_integer(), Context :: z:context()) ->
                     {ok, map()} | {error, term()}.
...
observe_validate_query_args(#validate_query_args{  }, {ok, Qs}, _Context) ->
    lists:foldr(fun ({K, #fileuploader{ name = Name }}, {ok, Acc}) ->
