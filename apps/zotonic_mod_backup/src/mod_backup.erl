...
start_backup(IsFullBackup, Context) ->
    gen_server:call(
        z_utils:name_for_site(?MODULE, z_context:site(Context)), {start_backup, IsFullBackup}).

%% @doc List all backups present.  Newest first.
list_backups(Context) ->
    InProgress =
        gen_server:call(
            z_utils:name_for_site(?MODULE, z_context:site(Context)), in_progress_start),
...
handle_info(periodic_backup, State) ->
    ...
    case z_convert:to_bool(
             m_config:get_value(mod_backup, daily_dump, State#state.context))
    of
        true ->
...
handle_info({'EXIT', Pid, _Error}, State) ->
    case State#state.backup_pid of
        Pid ->
            ...
            Name =
                z_convert:to_list(
                    z_datetime:format(State#state.backup_start, "Ymd-His", State#state.context)),
...
maybe_daily_dump(State) ->
    ...
    case Time >= {3, 0, 0} andalso Time =< {7, 0, 0} of
        true ->
            DoStart =
                case list_backup_files(State#state.context) of
                    [{_, LastBackupDate, _IsFull} | _] ->
                        LastBackupDate < {Date, {0, 0, 0}};
                    [] ->
                        true
                end,
            case DoStart of
...
do_backup(Name, IsFullBackup, State) ->
    ...
    spawn_link(fun() ->
                  do_backup_process(Name, IsFullBackup, State#state.context)
               end).
...
name(Context) ->
    iolist_to_binary([atom_to_list(z_context:site(Context)),
                      "-",
                      z_datetime:format(
                          calendar:universal_time(), "Ymd-His", Context)]).
...
pg_dump(Name, Context) ->
    ...
    Result =
        case os:cmd(binary_to_list(iolist_to_binary(Command))) of
            [] ->
                ok;
            Output ->
                ?LOG_WARNING("backup: pg_dump error for ~p: ~s", [z_context:site(Context), Output]),
...
archive(Name, Context) ->
    ...
    case filelib:is_dir(ArchiveDir) of
        true ->
            ...
            Command = lists:flatten([archive_cmd(), " -c -z ", "-f '", DumpFile, "' ", "-C '", ArchiveDir, "' ", " ."]),
            erlang:spawn(fun() ->
                            timer:sleep(1000),
                            z_mqtt:publish(<<"model/backup/event/backup">>,
                                           #{ status => <<"archive_backup_started">> },
                                           Context)
                         end),
            case os:cmd(Command) of
                "" ->
                    ok;
                Output ->
                    ?LOG_WARNING("backup: tar error for ~p: ~s", [z_context:site(Context), Output]),
                    {error, files_archive}
            end;
        false ->
            %% No files uploaded
...
archive_cmd() ->
    z_convert:to_list(
        z_config:get(tar, "tar")).

db_dump_cmd() ->
    z_convert:to_list(
        z_config:get(pg_dump, "pg_dump")).
...
check_configuration() ->
    ...
    [{ok, is_list(Db) and is_list(Tar)}, {db_dump, Db}, {archive, Tar}].
