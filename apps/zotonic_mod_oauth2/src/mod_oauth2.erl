...
try_bearer(Token, Context) ->
    case m_oauth2:decode_bearer_token(Token, Context) of
        ...
        {error, Reason} ->
            ...
            ?LOG_INFO("Could not decode OAuth2 token, error ~p for ~p", [ Reason, Token ]),
            Context
    end.
...
try_token(#{
    ...
    case z_auth:is_enabled(UserId, Context) of
        ...
        false ->
            ...
            ?LOG_INFO("Authenticated OAuth2 request for disabled user ~p with token ~p", [ UserId, TokenId ]),
            Context
    end.
