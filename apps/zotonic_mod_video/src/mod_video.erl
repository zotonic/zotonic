...
is_video_process_needed(<<"video/mp4">>, File) ->
    ...
    not (is_orientation_ok(Info) andalso is_audio_ok(Info) andalso is_video_ok(Info));
is_video_process_needed(_Mime, _File) ->
    true.

is_orientation_ok(#{ <<"orientation">> := 1 }) ->
    true;
is_orientation_ok(#{ <<"orientation">> := undefined }) ->
    true;
is_orientation_ok(_) ->
    false.

is_audio_ok(#{ <<"audio_codec">> := <<"aac">> }) ->
    true;
is_audio_ok(#{ <<"audio_codec">> := undefined }) ->
    true;
is_audio_ok(#{ <<"audio_codec">> := _ }) ->
    false;
is_audio_ok(_) ->
    true.

is_video_ok(#{ <<"video_codec">> := <<"h264">> }) ->
    true;
is_video_ok(#{ <<"video_codec">> := undefined }) ->
    true;
is_video_ok(#{ <<"video_codec">> := _ }) ->
    false;
is_video_ok(_) ->
    true.
...
do_media_upload_preprocess(Upload, Context) ->
    case z_module_indexer:find(lib, ?TEMP_IMAGE, Context) of
        {ok, #module_index{ filepath = Filename }} ->
            ...
            PostFun =
                fun(InsId, InsMedium, InsContext) ->
...
observe_media_upload_props(#media_upload_props{
    ...
    Info2 =
        case z_video_preview:preview(FileAbs, Info) of
            {ok, TmpFile} ->
...
observe_media_stillimage(#media_stillimage{ props = #{ <<"mime">> := <<"video/mp4">> } = Props }, _Context) ->
    case z_convert:to_binary(
             maps:get(<<"preview_filename">>, Props, undefined))
    of
        <<>> ->
...
init([]) ->
    Element = {z_video_convert, {z_video_convert, start_link, []}, temporary, brutal_kill, worker, [z_video_convert]},
    Children = [Element],
...
ensure_job_queues_1() ->
    case jobs:queue_info(video_jobs) of
        undefined ->
            jobs:add_queue(video_jobs, [{regulators, [{counter, [{limit, 1}, {modifiers, [{cpu, 1}]}]}]}]);
        {queue, _} ->
            ok
    end,
    case jobs:queue_info(media_preview_jobs) of
        undefined ->
            jobs:add_queue(media_preview_jobs, [{regulators, [{counter, [{limit, 3}, {modifiers, [{cpu, 1}]}]}]}]);
        {queue, _} ->
            ok
    end.
...
post_insert_fun(Id, Medium, Upload, ProcessNr, Context) ->
    ...
    supervisor:start_child(
        z_utils:name_for_site(?SERVER, Context), [Task, z_context:prune_for_async(Context)]),
    ok.
...
convert_task(Task, Context) ->
    _ = supervisor:start_child(
            z_utils:name_for_site(?SERVER, Context), [Task, Context]),
    {delay, ?TASK_DELAY}.
