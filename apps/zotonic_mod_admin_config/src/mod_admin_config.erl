...
event(#submit{ message = {config_save, Args} }, Context) ->
    case z_acl:is_admin_editable(Context) of
        true ->
            Module =
                z_convert:to_atom(
                    proplists:get_value(module, Args, undefined)),
            ...
            z_render:wire(
                proplists:get_all_values(on_success, Args), Context1);
        false ->
        ...
        <<>> ->
            {ok, MsgNr} =
                z_email:send(#email{
                             ...
                             Context),
            Context1 =
                z_render:update("email_test_result",
                                #render{
                                    template = "_admin_config_email_test_result.tpl",
                                    vars = [{email, Email}, {msg_nr, MsgNr}]
                                },
                                Context),
            z_render:growl([?__("Sent email to", Context), " ", z_html:escape(Email)], Context1);
        undefined ->
            z_render:dialog(?__("View email status", Context), "_dialog_email_status.tpl", [{email, Email}], Context)
    end.
