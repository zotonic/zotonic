...
%% @doc Batch processing and submissions of mailings.
%% @end
...
send(Id, Context) ->
    try
        ...
        true = mod_mailinglist:is_allowed_to_send(
            maps:get(<<"mailinglist_id">>, Run),
            maps:get(<<"page_id">>, Run),
            Context
        ),
        case
            m_mailinglist_run:check_history(
                maps:get(<<"mailinglist_id">>, Run),
                maps:get(<<"page_id">>, Run),
                maps:get(<<"options">>, Run, []),
                Context
            )
        of
            ok -> ok;
        ...
        throw:history_expired ->
            m_mailinglist_run:fail(Id, history_expired, Context);
        error:{badmatch, {error, history_expired}} ->
            m_mailinglist_run:fail(Id, history_expired, Context);
        Class:Reason:Stack ->
            ?LOG_ERROR(#{
                in => mod_mailinglist,
                text => <<"Mailing run interrupted">>,
                run_id => Id,
                result => Class,
                reason => Reason,
                stack => Stack
            ...
            z_db:q(
                "update mailinglist_run set status='interrupted', modified=now(),
                error='Sending was interrupted. Resume to process recipients not yet submitted.'
                where id=$1 and status <> 'cancelled' and details_expired is null",
                [Id],
                Context
            ),
...
prepare(#{<<"id">> := Id} = Run, Context) ->
    ...
    case
        z_db:q1(
            "select exists(select 1 from mailinglist_run_recipient where run_id=$1)", [Id], Context
        )
    of
        true ->
            ok;
        false ->
            ...
            ok = z_db:transaction(
                fun(Ctx) ->
                    case
                        z_db:q1(
                            "select status from mailinglist_run where id=$1 for update", [Id], Ctx
                        )
                    of
                        <<"cancelled">> ->
                            ok;
                        _ ->
                            maps:foreach(
                                fun(Email, Recipient) ->
                                    snapshot(RunWithHistory, Email, Recipient, Ctx)
                                end,
                                Recipients
                            )
                    end
...
save_content(#{<<"id">> := Id, <<"page_id">> := Page, <<"mailinglist_id">> := List}, Context) ->
    Languages = z_db:q(
        "select distinct rr.language from mailinglist_run_recipient rr
        where rr.run_id=$1 and rr.status='pending'
        and not exists(select 1 from mailinglist_run_content c where c.run_id=$1 and c.language=rr.language)",
        [Id],
        Context
    ),
    lists:foreach(
        fun({Language}) ->
            ...
            Vars = [
                {id, Page},
                {list_id, List},
                {email_from, m_mailinglist:get_email_from(List, Ctx)},
                {email_language, Language},
                {mailinglist_run_id, Id},
                {is_mailing_snapshot, true}
            ],
            ...
            z_db:transaction(
                fun(Tx) ->
                    case
                        z_db:q1(
                            "select status from mailinglist_run where id=$1
                    and details_expired is null for update",
                            [Id],
                            Tx
                        )
                    of
                        State when State =:= <<"sending">>; State =:= <<"preparing">> ->
                            z_db:q(
                                "insert into mailinglist_run_content(run_id,language,html) values ($1,$2,$3)
                        on conflict (run_id,language) do nothing",
                                [Id, Language, iolist_to_binary(Html)],
                                Tx
                            );
                        _ ->
                            ok
...
resolve(Run, Email0, Recipient, Context) ->
    Email =
        case Email0 of
            undefined -> <<>>;
            _ -> z_convert:to_binary(Email0)
        end,
    {Rsc, Pref} =
        case Recipient of
            I when is_integer(I) -> {I, m_rsc:p_no_acl(I, pref_language, Context)};
            M when is_map(M) ->
                {maps:get(<<"rsc_id">>, M, undefined), maps:get(<<"pref_language">>, M, undefined)}
        end,
...
snapshot(Run, Email0, Recipient, Context) ->
    ...
    ok = m_mailinglist_run:add_recipient(
        maps:get(<<"id">>, Run), Email, Rsc, Actual, State, Reason, Context
    ).
...
eligibility(Run, Email, Pref, Lang, Available, Context) ->
    ...
    case {Email, Match} of
        {<<>>, _} ->
            {<<"skipped">>, <<"Missing email address">>};
        {_, false} ->
            {<<"skipped">>, <<"Language mismatch">>};
        _ ->
...
previous(#{<<"is_test">> := true, <<"send_mode">> := Mode}, _, _, _) when Mode =/= <<"failed">> ->
    {<<"pending">>, undefined};
previous(#{<<"send_mode">> := <<"all">>}, _, _, _) ->
    {<<"pending">>, undefined};
previous(Run, Email, Lang, _Context) ->
    ...
    case {maps:get(<<"send_mode">>, Run), Sent, Failed} of
        {_, true, _} ->
            {<<"skipped">>, <<"Previously sent or already pending">>};
        {<<"failed">>, false, false} ->
            {<<"skipped">>, <<"No previous failure">>};
        {<<"new">>, false, true} ->
            {<<"skipped">>, <<"Previous failure; use retry failed recipients">>};
        _ ->
            {<<"pending">>, undefined}
    end.
...
batches(#{<<"id">> := Id} = Run, Context) ->
    case z_db:q1("select status from mailinglist_run where id=$1", [Id], Context) of
        ...
        _ ->
            {ok, Rows} = z_db:qmap(
                "select * from mailinglist_run_recipient
                where run_id=$1 and status='pending' order by id limit 100",
                [Id],
                Context
            ),
            case Rows of
...
submit(Run, Recipient, Context) ->
    ...
    MsgId = z_ids:id(32),
    Reserved = z_db:transaction(
        fun(Ctx) ->
            %% Serialize duplicate checks for concurrent runs of the same page/list.
            z_db:q(
                "select pg_advisory_xact_lock($1,$2)",
                [maps:get(<<"page_id">>, Run), maps:get(<<"mailinglist_id">>, Run)],
                Ctx
            ),
            Status = z_db:q1(
                "select status from mailinglist_run where id=$1 for update",
                [maps:get(<<"id">>, Run)],
                Ctx
            ),
            Current = z_db:q1(
                "select status from mailinglist_run_recipient where id=$1 for update", [Id], Ctx
            ),
            case {Status, Current} of
                {<<"cancelled">>, _} ->
                    m_mailinglist_run:transition(
                        Id, <<"cancelled">>, <<"Cancelled before submission">>, Ctx
                    ),
                    false;
                {S, <<"pending">>} when S =:= <<"sending">>; S =:= <<"preparing">> ->
                    %% Other runs' pending snapshots must not block each other forever.
                    Duplicate =
                        maps:get(<<"send_mode">>, Run) =/= <<"all">> andalso
                            not maps:get(<<"is_test">>, Run) andalso
                            m_mailinglist_run:previous(
                                Run,
                                Email,
                                Lang,
                                [<<"submitting">>, <<"queued">>, <<"retrying">>, <<"sent">>],
                                Ctx
                            ),
                    case Duplicate of
                        true ->
                            m_mailinglist_run:transition(
                                Id, <<"skipped">>, <<"Previously sent or already pending">>, Ctx
                            ),
                            false;
                        false ->
                            m_mailinglist_run:transition(Id, <<"submitting">>, undefined, Ctx),
                            z_db:q(
                                "insert into mailinglist_run_message(message_nr,recipient_id) values ($1,$2)",
                                [MsgId, Id],
                                Ctx
                            ),
                            true
        ...
        true ->
            ...
            Mail = #email{
                ...
                vars = [
                    ...
                    {email, Email},
                    {email_language, Lang},
                    {mailinglist_run_id, maps:get(<<"id">>, Run)}
                ],
            ...
            case z_email_server:send_queued(MsgId, Mail, z_context:set_language(Lang, Context)) of
                {ok, _} ->
                    m_mailinglist_run:message(MsgId, <<"queued">>, false, 0, undefined, Context);
                {error, Reason} ->
                    m_mailinglist_run:message(MsgId, <<"failed">>, true, 0, Reason, Context)
            end;
        false ->
            ok;
        Other ->
...
normalized_candidates(List, Context) ->
    maps:fold(
        fun(Email, R, Acc) ->
            Key =
                case m_mailinglist:normalize_email(Email) of
        ...
        #{},
        z_mailinglist_recipients:list_candidates(List, Context)
    ).
...
review(List, Page, Options, Context) ->
    ...
    Run = #{
        ...
        <<"options">> => Options,
        <<"parent_id">> => proplists:get_value(parent_id, Options),
        <<"language">> => proplists:get_value(language, Options, <<>>),
        <<"fallback_language">> => proplists:get_value(
            fallback_language, Options, m_mailinglist_run:fallback(Page, Context)
        ),
    ...
    maps:fold(
        fun(Email, R, Acc) ->
            ...
            Reasons =
                case State of
                    <<"skipped">> ->
                        maps:update_with(Reason, fun(N) -> N + 1 end, 1, maps:get(reasons, Acc));
                    _ ->
                        maps:get(reasons, Acc)
                end,
            Acc#{counts => Counts, reasons => Reasons}
        end,
        #{counts => #{}, reasons => #{}},
        recipients(Run, List, Context)
    ).
...
with_history(Run, Context) ->
    ...
    ok = m_mailinglist_run:check_history(
        maps:get(<<"mailinglist_id">>, Run),
        maps:get(<<"page_id">>, Run),
        maps:get(<<"options">>, Run, []),
        Context
    ),
    History = maps:from_list([
        {{maps:get(<<"email">>, R), maps:get(<<"language">>, R)}, {
            maps:get(<<"sent">>, R), maps:get(<<"failed">>, R)
        }}
     || R <- Rows
    ]),
    Run#{history => History}.
