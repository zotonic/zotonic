<table class="table">
    ...
    <tbody>
            ...
            <tr>
                <td>
                    ...
                    <br><small>{{ run.created|date:"Y-m-d H:i" }} ·
                        ...
                        {% else %}
                            {_ Recipient preference _}
                        {% endif %}
                    </small>
                </td>
                <td>
                    <a href="{% url admin_mailing_run run_id=run.id %}">
        ...
        {% empty %}
            <tr>
                <td colspan="2">
                    <span class="text-muted">{_ No mailings yet. Send a mailing from a page’s edit screen to see its progress here. _}</span>
                </td>
            </tr>
        {% endfor %}
    </tbody>
