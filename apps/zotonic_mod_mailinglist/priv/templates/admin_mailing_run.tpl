...
{% block content %}
    ...
    {% if m.mailinglist_run.run[q.run_id] as run %}
        ...
        <section class="widget mailing-run-section" aria-labelledby="mailing-recipients-title">
            <div class="widget-header">
                <h3 id="mailing-recipients-title">{_ Recipient results _}</h3>
            </div>
            <div class="widget-content">
                ...
                <form method="get" class="mailing-run-recipient-filter">
                    <label>{_ Recipient status _}
                        <select name="recipient_status" class="form-control">
                            <option value="">{_ All statuses _}</option>
                            {% for state in [
                                    "pending","submitting","queued","retrying","sent",
                                    "failed","bounced","skipped","cancelled"
                                ]
                            %}
                                <option value="{{ state }}" {% if q.recipient_status == state %}selected{% endif %}>
                                    {% include "_mailing_recipient_status.tpl" status=state %}
                                </option>
                            {% endfor %}
                        </select>
                    </label>
    ...
    {% else %}
        <p>{_ This mailing is unavailable or you do not have permission to view it. _}</p>
    {% endif %}
{% endblock %}
