...
<tr><td>{{ recipient.email|escape }}</td><td>{{ recipient.language|escape }}</td><td>{% include "_mailing_recipient_status.tpl" status=recipient.status %}</td><td>{% include "_mailing_skip_reason.tpl" reason=recipient.reason %}</td></tr>
{% empty %}<tr><td colspan="4">{% if run.prepared %}{_ No recipients match this filter. _}{% else %}{_ Recipients will appear when preparation starts. _}{% endif %}</td></tr>{% endfor %}
</tbody></table>
{% if recipients|length == 100 %}{% with recipients|last as last_recipient %}
