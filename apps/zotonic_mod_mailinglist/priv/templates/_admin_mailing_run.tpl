...
<section class="widget mailing-run-section" aria-labelledby="mailing-overview-title">
    ...
    <p><span class="text-muted">{_ Language: _}</span>
        ...
        <strong>{% if run.sender_id %}<a href="{% url admin_edit_rsc id=run.sender_id %}">{% include "_name.tpl" id=run.sender_id %}</a>{% else %}{_ Unknown _}{% endif %}</strong>
    </p>
    {% if run.parent_id %}
        <p>{_ This is a resend of an earlier mailing. _} <a href="{% url admin_mailing_run run_id=run.parent_id %}">{_ View original mailing _}</a></p>
    {% endif %}

    {% include "_mailing_run_summary.tpl" stats=run.stats %}

    {% if run.status == "empty" %}
        <p class="alert alert-warning">{_ Nobody received this mailing. Check the recipient xbelow for the reasons, then change the language or recipient selection. _}</p>
    {% endif %}
    {% if run.status == "cancelled" %}
        <p>{_ Sending stopped. _} {{ run.stats.sent|default:0 }} {_ sent; _}
        {{ run.stats.queued|default:0 }} {_ already queued; _} {{ run.stats.retrying|default:0 }} {_ still being retried; _}
        {{ run.stats.submitting|default:0 }} {_ awaiting confirmation; _} {{ run.stats.cancelled|default:0 }} {_ will not be sent. _}</p>
    {% endif %}
    </div>
</section>
...
<section class="widget mailing-run-section" aria-labelledby="mailing-actions-title">
    ...
    <div class="mailing-run-actions">
        ...
        {% if run.error %}<details class="mailing-run-diagnostics"><summary>{_ Technical details _}</summary>{{ run.error|escape }}</details>{% endif %}
    </div>
</section>
{% endif %}
