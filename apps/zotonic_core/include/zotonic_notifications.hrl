...
%% Type: first
-record(security_headers, {headers :: [{binary(), binary()}]}).
%% @doc Set CORS headers on the HTTP response.
%% Type: first
-record(cors_headers, {headers :: [{binary(), binary()}]}).
...
-record(resource_headers, {id :: m_rsc:resource_id() | undefined}).
...
%% Type: notify
-record(module_activate, {module :: atom(), pid :: pid()}).
%% @doc A module has been stopped and deactivated.
%% Type: notify
-record(module_deactivate, {module :: atom()}).
...
%% Type: foldl
%% Return:: ``map()``
-record(logon_options, {payload = #{  } :: map()}).
...
-record(signup_url, {props = #{  } :: map(), signup_props = [] :: list()}).
...
-record(signup_check, {props = #{  } :: map(), signup_props = [] :: list()}).
%% @doc Signal that a user has been signed up (map, result is ignored)
%% Type: map
-record(signup_done, {id :: m_rsc:resource(), is_verified :: boolean(), props :: map(), signup_props :: list()}).
%% @doc Signal that a user has been confirmed. (map, result is ignored)
%% Type: notify
-record(signup_confirm, {id :: m_rsc:resource()}).
%% @doc Fetch the page a user is redirected to after signing up with a confirmed identity
%% Type: first
%% Return: a URL or ``undefined``
-record(signup_confirm_redirect, {id :: m_rsc:resource()}).
...
%% Type: notify
-record(comment_insert, {comment_id :: integer(), id :: m_rsc:resource_id()}).
%% @doc Notify that the session's language has been changed
%% Type: notify
-record(language, {language :: atom()}).
%% @doc Set the language of the context to a user's prefered language
%% Type: first
-record(set_user_language, {id :: m_rsc:resource_id()}).
...
%% Type: foldl
-record(url_rewrite, {dispatch :: atom(), args = [] :: list()}).
%% @doc Rewrite a URL before it will be dispatched using the z_sites_dispatcher
%% Type: foldl
-record(dispatch_rewrite, {is_dir = false :: boolean(), path = <<>> :: binary(), host}).
...
%% Type: foldl
-record(admin_edit_blocks, {id :: m_rsc:resource_id()}).
%% @doc Used in the admin to process a submitted resource form
-record(admin_rscform, {id :: m_rsc:resource_id(), is_a :: [atom()]}).
...
%% Type: first
-record(menu_rsc, {id :: m_rsc:resource()}).
...
%% subscribers.
%% Type: map
-record(activity, {version = 1 :: pos_integer(), posted_time, actor, verb = post :: atom(), object, target}).
%% @doc Push a list of activities via a 'channel' (eg 'email') to a recipient.
%% The activities are a list of #activity{} records.
%% Type: first
-record(activity_send, {recipient_id, channel, queue, activities = [] :: list()}).
...
%% Type: first
-record(email_is_blocked, {recipient :: binary()}).
%% @doc Email status notification, sent when the validity of an email recipient changes
%% Type: notify
-record(email_status, {recipient :: binary(), is_valid :: boolean(), is_final :: boolean(), is_manual :: boolean()}).
...
%% Type: notify
-record(email_bounced, {message_nr :: binary() | undefined, recipient :: binary() | undefined}).
...
-record(rsc_insert, {props :: m_rsc:props()}).
%% @doc Map to signal merging two resources. Move any information from the loser to the
%% winner. The loser will be deleted.
%% Type: map
-record(rsc_merge, {winner_id :: m_rsc:resource_id(), loser_id :: m_rsc:resource_id(), is_merge_trans :: boolean()}).
...
-record(rsc_update, {action :: insert | update, id :: m_rsc:resource_id(), props :: m_rsc:props()}).
...
%% Type: foldl
-record(pivot_fields, {id :: m_rsc:resource_id(), raw_props :: m_rsc:props()}).
%% @doc Signal that a resource pivot has been done.
%% Type: notify
-record(rsc_pivot_done, {id :: m_rsc:resource_id(), is_a = [] :: [atom()]}).
%% @doc Sanitize an HTML element.
%% Type: foldl
-record(sanitize_element, {element :: {binary(), [{binary(), binary()}], list()}, stack :: list()}).
...
-record(sanitize_embed_url, {hostpath :: binary()}).
%% @doc Check if a user is the owner of a resource.
%% ``id`` is the resource id.
%% Type: first
%% Return: ``true``, ``false`` or ``undefined`` to let the next observer decide
-record(acl_is_owner, {id :: m_rsc:resource_id(), creator_id :: m_rsc:resource_id(), user_id :: m_rsc:resource_id()}).
...
-record(acl_is_allowed, {action :: view | update | delete | insert | use | atom(), object :: term()}).
...
-record(acl_is_allowed_prop, {action :: view | update | delete | insert | atom(), object :: term(), prop :: binary()}).
...
-record(acl_logon, {id :: m_rsc:resource_id(), options :: map()}).
...
%% Type: foldl
%% Return: ``map()``
-record(auth_options_update, {request_options = #{  } :: map()}).
...
-record(auth_client_logon_user, {user_id :: m_rsc:resource_id(), url = <<"#reload">> :: binary() | undefined}).
...
%% Type: first
%% Return: ``ok | {error, term()}``
-record(auth_client_switch_user, {user_id :: m_rsc:resource_id()}).
...
%% Type: foldl
%% Return: ``#context{}``
-record(session_context, {request_type :: http | mqtt, payload = undefined :: undefined | term()}).
...
-record(page_url, {id :: m_rsc:resource_id(), is_a :: [atom()]}).
...
-record(search_query,
        {name = undefined :: binary() | undefined,
         ...
         offsetlimit :: {Offset :: pos_integer(), Limit :: pos_integer()},
         % Deprecated {searchname, [..]} syntax.
         search = undefined :: {SearchName :: atom(), SearchProps :: list()} | undefined}).
...
%% Return: return value is ignored
-record(edge_insert,
        {subject_id :: m_rsc:resource(), predicate :: atom(), object_id :: m_rsc:resource(), edge_id :: pos_integer()}).
...
%% Type: notify
%% Return: return value is ignored
-record(m_config_update, {module :: atom(), key :: term(), value :: term()}).
%% @doc Site configuration parameter was changed
%% Type: notify
%% Return: return value is ignored
...
-record(rsc_import_fetch, {uri :: binary()}).
...
%% Type: first
%% Return: ``ok | {error, term()}``.
-record(media_import_medium, {id :: m_rsc:resource_id(), medium :: map()}).
...
%% Type: notify
%% Return: return value is ignored
-record(rsc_query_item, {query_id :: m_rsc:resource_id(), match_id :: m_rsc:resource_id()}).
...
%% Type: map
-record(scomp_script_render, {is_nostartup = false :: boolean(), args = [] :: list()}).
...
-record(import_csv_definition, {basename :: binary(), filename :: file:filename_all()}).
...
-record(multiupload, {upload :: term() | string(), query_args = [] :: list()}).
...
-record(media_identify_file, {filename :: file:filename_all(), original_filename :: binary(), extension :: binary()}).
%% @doc Try to find a filename extension for a mime type (example: ".jpg")
%% Type: first
%% Return: Extension (for example ``<<".png">>``) or ``undefined``
-record(media_identify_extension, {mime :: binary(), preferred :: undefined | binary()}).
...
-record(media_stillimage, {id :: m_rsc:resource_id() | undefined, props :: z_media_identify:media_info()}).
...
%% Type: notify
%% Return: return value is ignored
-record(filewatcher, {verb :: modify | create | delete, file :: binary(), basename :: binary(), extension :: binary()}).
...
-record(export_resource_content_disposition,
        {dispatch :: atom(), id :: m_rsc:resource_id() | undefined, content_type :: binary()}).
%% @doc mod_export - Check if the resource or dispatch is visible for export.
%% Type: first
%% Return: ``true`` or ``false``
-record(export_resource_visible, {dispatch :: atom(), id :: m_rsc:resource_id() | undefined}).
%% @doc mod_export -
%% Type: first
%% Return: ``{ok, "text/csv"})`` for the dispatch rule/id export.
-record(export_resource_content_type, {dispatch :: atom(), id :: m_rsc:resource_id() | undefined}).
...
-record(export_resource_header, {dispatch :: atom(), id :: m_rsc:resource_id() | undefined, content_type :: binary()}).
...
-record(export_resource_data,
        {dispatch :: atom(), id :: m_rsc:resource_id() | undefined, content_type :: binary(), state :: term()}).
...
-record(url_fetch_options, {host :: binary(), url :: binary(), options :: z_url_fetch:options()}).
