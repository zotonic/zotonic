...
-record(context,
        {%% Cowboy request data (only set when this context is used because of a request)
         ...
         translation_table :: pid() | atom()}).        %% End TODO
...
-record(multipart_form,
        {name, data, filename, tmpfile, tmpmonitor, content_type, content_length, file, files = [], args = []}).
...
-record(rsc_list, {list :: [m_rsc:resource_id()]}).
%% @doc Menu structures, a tree with a resource as a node.
-record(rsc_tree, {id = undefined :: m_rsc:resource() | undefined, tree = [] :: [#rsc_tree{  }]}).
...
-record(search_sql,
        {select :: iodata(),
         ...
         extra = [] :: list(),
         assoc = false :: boolean(),
         search_sql_terms = undefined ::
             list() | undefined}). % Optional, if search was built using #search_sql_term{} records
...
-record(dispatch_redirect, {location = <<>> :: binary(), is_permanent = false :: boolean()}).
%% Record could be returned by #dispatch notification
...
%% Record could be returned by #dispatch notification
-record(dispatch_rules, {rules :: z_sites_dispatcher:site_dispatch_list() | undefined}).
...
-record(acl_rsc, {id :: m_rsc:resource_id() | undefined, category :: atom(), props = #{  } :: map()}).
%% Edge insert and delete
-record(acl_edge, {subject_id :: m_rsc:resource(), predicate :: pos_integer() | atom(), object_id :: m_rsc:resource()}).
%% Media uploads
-record(acl_media, {mime = <<"binary/octet-stream">> :: binary(), size = undefined :: undefined | non_neg_integer()}).
%% MQTT acl check, called via the normal acl notifications.
%% Actions for these checks: subscribe, publish
-record(acl_mqtt, {topic :: [binary()], is_wildcard :: boolean(), packet :: mqtt_packet_map:mqtt_packet()}).
%% ACL notifications
...
-record(render,
        {template :: template_compiler:template(), is_all = false :: boolean(), vars = [] :: proplists:proplist()}).
%% @doc Data import definition. See also mod_import_csv.
-record(import_data_def,
        {colsep = $\t :: 0..255, skip_first_row = true :: boolean(), columns = [] :: list(), importdef}).
