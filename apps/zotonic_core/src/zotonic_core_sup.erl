...
start_link(Options) ->
    lists:foreach(fun({K, V}) ->
                     application:set_env(zotonic_core, K, V)
                  end,
                  Options),
...
init([]) ->
    ...
    Processes =
        [% Ring buffer for http request logs
         ...
         {z_email_server, {z_email_server, start_link, []}, permanent, 5000, worker, [z_email_server]},
         {z_file_sup, {z_file_sup, start_link, []}, permanent, 5000, supervisor, dynamic},
         % Sites supervisor, starts all enabled sites
         {z_sites_manager_sup, {z_sites_manager_sup, start_link, []}, permanent, 10100, supervisor, dynamic},
...
spawn_delayed_status() ->
    erlang:spawn(fun() ->
                    ...
                    {Running, Other} =
                        lists:partition(fun({_Site, Status}) ->
                                           Status =:= running
                                        end,
                                        SitesStatus),
                    lists:map(fun ({Site, running}) when Site =/= zotonic_site_status ->
...
ensure_job_queues() ->
    ...
    ensure_job_queue(zotonic_singular_job, [{regulators, [{counter, [{limit, 1}]}]}]),
    ok.
