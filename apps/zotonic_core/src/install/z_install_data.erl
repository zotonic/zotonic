...
install(Site, Context) ->
    ...
    ?LOG_INFO("~p: Install done.", [Site]),
    ok.

install_category(C) ->
    ?LOG_INFO("Inserting categories"),
...
install_rsc(C) ->
    ?LOG_INFO("Inserting base resources (admin, etc.)"),
    Rsc = [
...
install_identity(C) ->
    ?LOG_INFO("Inserting username for the admin"),
