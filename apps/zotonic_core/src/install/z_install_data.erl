...
install_category(C) ->
    ...
    %% Bootstrap runs before ACL observers are ready. Keep privacy closed (-1)
    %% and mark it as derived so z_rsc_defaults can apply module defaults later.
    %% Content groups are likewise assigned by the modules once they are ready.
    ...
    Cats = [
        {101,undefined,  1,1,1,1, other,       true,  undefined,                                   [{title, #trans{ tr = [{en, <<"Uncategorized">>}, {nl, <<"Zonder categorie">>}]}}] },

        {104,undefined,  2,1,2,4, text,        false, "http://purl.org/dc/dcmitype/Text",          [{title, #trans{ tr = [{en, <<"Text">>}, {nl, <<"Tekst">>}]}}] },
            {106,104,    3,2,3,3, article,     false, undefined,                                   [{title, #trans{ tr = [{en, <<"Article">>}, {nl, <<"Artikel">>}]}}] },
                {109,106,4,3,4,4, news,        false, undefined,                                   [{title, #trans{ tr = [{en, <<"News">>}, {nl, <<"Nieuws">>}]}}] },

        {102,undefined,  5,1,5,5, person,      true,  undefined,                                   [{title, #trans{ tr = [{en, <<"Person">>}, {nl, <<"Persoon">>}]}}] },

        {119,undefined,  6,1,6,7, location,    false, undefined,                                   [{title, #trans{ tr = [{en, <<"Location">>}, {nl, <<"Locatie">>}]}}] },
            {107,119,    7,2,7,7, website,     false, undefined,                                   [{title, #trans{ tr = [{en, <<"Website">>}, {nl, <<"Website">>}]}}] },

        {108, undefined, 8,1,8,8, event,       false, "http://purl.org/dc/dcmitype/Event",         [{title, #trans{ tr = [{en, <<"Event">>}, {nl, <<"Evenement">>}]}}] },

        {103,undefined,  9,1,9,9, artifact,    false, "http://purl.org/dc/dcmitype/PhysicalObject",[{title, #trans{ tr = [{en, <<"Artifact">>}, {nl, <<"Artefact">>}]}}] },

        {110,undefined,  10,1,10,14, media,       true,  "http://purl.org/dc/dcmitype/Image",         [{title, #trans{ tr = [{en, <<"Media">>}, {nl, <<"Media">>}]}}] },
            {111,110,    11,2,11,11, image,       true,  "http://purl.org/dc/dcmitype/StillImage",    [{title, #trans{ tr = [{en, <<"Image">>}, {nl, <<"Afbeelding">>}]}}] },
            {112,110,    12,2,12,12, video,       true,  "http://purl.org/dc/dcmitype/MovingImage",   [{title, #trans{ tr = [{en, <<"Video">>}, {nl, <<"Video">>}]}}] },
            {113,110,    13,2,13,13, audio,       true,  "http://purl.org/dc/dcmitype/Sound",         [{title, #trans{ tr = [{en, <<"Audio">>}, {nl, <<"Geluid">>}]}}] },
            {114,110,    14,2,14,14, document,    true,  undefined,							         [{title, #trans{ tr = [{en, <<"Document">>}, {nl, <<"Document">>}]}}] },

        {120,undefined,  15,1,15,16, collection,  false, "http://purl.org/dc/dcmitype/Collection",    [{title, #trans{ tr = [{en, <<"Collection">>}, {nl, <<"Collectie">>}]}}] },
            {121,120,    16,2,16,16, 'query',     false, "http://purl.org/dc/dcmitype/Dataset",       [{title, #trans{ tr = [{en, <<"Search query">>}, {nl, <<"Zoekopdracht">>}]}}] },

        {122,undefined,  17,1,17,18, categorization,true,undefined,                                   [{title, #trans{ tr = [{en, <<"Categorization">>}, {nl, <<"Categorisatie">>}]}}] },
            {123,122,    18,2,18,18, keyword,     true,  undefined,                                   [{title, #trans{ tr = [{en, <<"Keyword">>}, {nl, <<"Trefwoord">>}]}}] },

        % 115. Meta (see above)
            % 116. Category (see above)
            {117,115,    92,2,92,92, predicate,   true,  undefined,                                   [{title, #trans{ tr = [{en, <<"Predicate">>},     {nl, <<"Predikaat">>}]}}] }
...
install_predicate(C) ->
    ...
    Preds = [
        ...
        [ 300, true,   "about",    "http://www.w3.org/1999/02/22-rdf-syntax-ns#about",  json_props([{reversed, false},{title, #trans{ tr = [{en,<<"About">>},    {nl,<<"Over">>}]}}])],
        [ 301, true,   "author",   "http://purl.org/dc/terms/creator",                  json_props([{reversed, false},{title, #trans{ tr = [{en,<<"Author">>},   {nl,<<"Auteur">>}]}}])],
        [ 303, true,   "relation", "http://purl.org/dc/terms/relation",                 json_props([{reversed, false},{title, #trans{ tr = [{en,<<"Relation">>}, {nl,<<"Relatie">>}]}}])],
        [ 304, true,   "depiction","http://xmlns.com/foaf/0.1/depiction",               json_props([{reversed, false},{title, #trans{ tr = [{en,<<"Depiction">>},{nl,<<"Afbeelding">>}]}}])],
        [ 308, true,   "subject",  "http://purl.org/dc/elements/1.1/subject",           json_props([{reversed, false},{title, #trans{ tr = [{en,<<"Keyword">>},  {nl,<<"Trefwoord">>}]}}, {is_connect_checkbox, true}])],
        [ 309, true,   "hasdocument", "http://zotonic.net/predicate/hasDocument",       json_props([{reversed, false},{title, #trans{ tr = [{en,<<"Document">>}, {nl,<<"Document">>}]}}])],
		[ 310, true,   "haspart",  "http://purl.org/dc/terms/hasPart",					json_props([{reversed, false},{title, #trans{ tr = [{en,<<"Contains">>}, {nl,<<"Bevat">>}]}}])]
    ],
...
%% @doc Encode bootstrap properties as a JSON object, preserving translated values.
json_props(Props) ->
    ?DB_PROPS_JSON(maps:from_list(Props)).
