...
props_test() ->
    {ok, Ps} =
        z_props:from_qs([{<<"dt:y:0:publication_start">>, <<"2008">>},
    ...
    #{
        ...
        Ps,
    %%
    {ok, #{ <<"date_end">> := {{2009, 12, 31}, {23, 59, 59}} }} =
        z_props:from_qs([{<<"dt:y:0:date_end">>, <<"2009">>}]),
    %%
    ...
    %%
    {ok, #{ <<"title">> := #trans{ tr = [{en, <<"Hello">>}, {nl, <<"Hallo">>}] } }} =
        z_props:from_qs([{<<"title$nl">>, <<" Hallo ">>}, {<<"title$en">>, <<"Hello ">>}]),
    %%
    {ok,
     #{
         <<"a">> :=
        ...
        z_props:from_qs([{<<"a.b.c">>, 4}, {<<"a.d">>, 5}]),
    %%
    {ok, #{ <<"a">> := [1, 2, 3] }} = z_props:from_qs([{<<"a[]">>, 1}, {<<"a[]">>, 2}, {<<"a[]">>, 3}]),
    %%
    {ok,
     #{
         <<"a">> :=
        ...
        z_props:from_qs([{<<"a[].b">>, 1}, {<<"a[].c">>, 2}]),
    %%
    {ok, #{ <<"a">> := [#{ <<"b">> := 1 }, #{ <<"c">> := 2 }] }} =
        z_props:from_qs([{<<"a[].b">>, 1}, {<<"a[].">>, <<>>}, {<<"a[].c">>, 2}]),
    %%
    {ok, #{ <<"a">> := [#{ <<"b">> := #trans{ tr = [{en, <<"Hello">>}, {nl, <<"Hallo">>}] } }, #{ <<"c">> := 2 }] }} =
        z_props:from_qs([{<<"a[].b$nl">>, <<"Hallo">>},
                         {<<"a[].b$en">>, <<"Hello">>},
                         {<<"a[].">>, <<>>},
                         {<<"a[].c">>, 2}]),
    %%
    ...
    #{ <<"a">> := #trans{ tr = [{en, <<>>}] } } =
        z_props:prune_languages(#{ <<"a">> => #trans{ tr = [{en, <<>>}] } }, [en, nl]),
    #{ <<"a">> := [#trans{ tr = [{en, <<>>}] }, #{ <<"a">> := #trans{ tr = [{nl, <<>>}, {en, <<>>}] } }] } =
