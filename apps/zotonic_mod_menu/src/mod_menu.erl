...
event(#postback_notify{
    ...
    case Kind of
        ...
        menu ->
            ...
            z_notifier:notify_sync(#menu_save{
                                       id = m_rsc:rid(RootId, Context1),
                                       tree = Tree1
                                   },
                                   Context1),
            Context1;
    ...
    case m_rsc:is_visible(FromId, Context) of
        true ->
            ...
            case m_rsc:duplicate(FromId, [{title, NewTitle}], Context) of
                {ok, NewId} ->
                    Template =
                        case proplists:get_value(<<"template">>, Data) of
                            undefined ->
                                "_menu_edit_item.tpl";
                            <<>> ->
                                "_menu_edit_item.tpl";
                            Tpl ->
                                z_convert:to_list(Tpl)
                        end,
                    {Html, Context1} =
                        z_template:render_to_iolist({cat, Template}, [{id, NewId}, {editable, true}], Context),
...
handle_cmd(<<"delete">>, Data, Context) ->
    ...
    case m_rsc:is_deletable(Id, Context) of
        ...
        false ->
            z_render:wire({alert, [{text, ?__("Sorry, you are not allowed to delete this.", Context)}]}, Context)
    end.
...
make_copy_title(#trans{ tr = Ts }, Context) ->
    #trans{ tr = [{Lang, make_copy_title(Title, z_context:set_language(Lang, Context))} || {Lang, Title} <- Ts] }.
...
hierarchy_edge(RootId, Predicate, Tree, Context) ->
    ...
    z_notifier:notify(#hierarchy_updated{
                          root_id = RootId,
                          predicate = PredName
                      },
                      Context),
...
move_edges(RootId, Tree, Pred, Context) ->
    ...
    case z_acl:rsc_editable(RootId, Context) of
        true ->
            ok = m_edge:replace(RootId, Pred, Wanted, Context);
        false ->
            ignore
    end,
...
unpack(S, Acc) ->
    {Id, S1} =
        lists:splitwith(fun (C) when C == $[; C == $]; C == $, ->
                                false;
                            (_) ->
                                true
                        end,
...
create_new_if_needed(Id, Context) ->
    ...
    Context1 =
        z_render:wire([{script, [{script, ["$('#", Id, "').attr('id','", OuterId, "');"]}]},
                       {dialog_edit_basics, [{id, RscId}, {target, InnerId}, {template, "_menu_edit_item.tpl"}]},
                       {update,
                        [{target, OuterId}, {menu_id, InnerId}, {id, RscId}, {template, "_menu_edit_item.tpl"}]}],
                      Context),
    {RscId, Context1}.
...
get_menu(Id, Context) ->
    case m_rsc:p(Id, menu, Context) of
        ...
        <<>> ->
            [];
        Menu ->
            remove_invisible(validate(Menu, []), [], Context)
    end.
...
validate([#rsc_tree{
          ...
          | Ms],
         Acc)
    when is_integer(Id); is_atom(Id); is_binary(Id) ->
    Sub1 = validate(Sub, []),
...
remove_invisible([#rsc_tree{
    ...
    case m_rsc:is_visible(Id, Context) andalso m_rsc:exists(Id, Context) of
        true ->
            remove_invisible(Rest,
                             [#rsc_tree{
                                  id = Id,
                                  tree = remove_invisible(Sub, [], Context)
                              }
                              | Acc],
                             Context);
        false ->
            remove_invisible(Rest, Acc, Context)
    end;
% Old notations
...
-spec menu_flat(list() | undefined | <<>>, z:context()) ->
                   [{integer() | undefined, LevelIndex :: [integer()] | undefined, up | down | undefined}].
menu_flat(Menu, Context) ->
    menu_flat(Menu, 999, Context).

-spec menu_flat(list() | undefined | <<>>, integer(), z:context()) ->
                   [{integer() | undefined, LevelIndex :: [integer()] | undefined, up | down | undefined}].
...
menu_flat([#rsc_tree{ id = MenuId } | Rest], 1, [Idx | PR], Acc, Context) ->
    [{m_rsc:rid(MenuId, Context), [Idx | PR], undefined}] ++ menu_flat(Rest, 1, [Idx + 1 | PR], [], Context) ++ Acc;
menu_flat([#rsc_tree{
               id = MenuId,
               tree = Children
           }
...
menu_subtree_1([#rsc_tree{
               ...
               CurrMenu,
               Context)
    when not is_integer(Id), Id =/= undefined ->
...
manage_schema(install, Context) ->
    #datamodel{
        categories = [{menu, categorization, #{ <<"title">> => <<"Page Menu">> }}],
        resources =
            [{main_menu,
              menu,
              #{
