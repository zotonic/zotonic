...
| `all_bytitle`                | Return all `{title,id}` pairs for a category, sorted on title.                   | cat                          |
| `all_bytitle_featured`       | Return all `{title,id}` pairs for a category, sorted on title, featured pages first | cat                          |
| `all_bytitle`                | Return all `{title,id}` pairs for a category without subcategories, sorted on title. | `cat_is`                     |
| `all_bytitle_featured`       | Return all `{title,id}` pairs for a category without subcategories, sorted on title, featured pages first. | `cat_is`                     |
| `match_objects`              | Returns a list of pages with similar object ids to the objects of the given resource with the given id. Accepts optional `cat` parameters for filtering on category. Optionally accepts a `predicate` parameter to only use the objects that are connected to the given `id` using the predicate or predicates. | `id`                           |
| `match_objects_cats`         | Returns a list of pages with similar object ids or categories. Accepts an optional `cat` parameter for filtering on category. | `id`                           |
