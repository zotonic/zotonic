        ...
        AuthCookie ->
            case decode_auth_token(AuthCookie, Context) of
                {ok, {UserId, Expires}} ->
                    ...
                    z_context:set(auth_expires, Expires, Context1);
                {error, _} ->
                    reset_auth_cookie(Context)
            end
    end.

-spec set_auth_cookie( m_rsc:resource_id(), z:context() ) -> z:context().
set_auth_cookie(UserId, Context) ->
    Cookie = encode_auth_token(UserId, Context),
    Options = [
        {path, <<"/">>},
        {http_only, true},
        {secure, true},
    ...
    Context1 = z_context:set_cookie(?AUTH_COOKIE, Cookie, Options, Context),
    z_context:set(auth_expires, session_expires(Context1), Context1).

-spec refresh_auth_cookie( z:context() ) -> z:context().
refresh_auth_cookie(Context) ->
    case z_context:get_cookie(?AUTH_COOKIE, Context) of
        undefined ->
...
reset_auth_cookie(Context) ->
    ...
    z_context:set_cookie(?AUTH_COOKIE, <<>>, Options, Context).

-spec encode_auth_token( m_rsc:resource_id(), z:context() ) -> binary().
encode_auth_token(UserId, Context) ->
    Term = {auth, UserId, user_secret(UserId, Context)},
    ExpTerm = termit:expiring(Term, session_expires(Context)),
    termit:encode_base64(ExpTerm, auth_secret(Context)).

-spec decode_auth_token( binary(), z:context() ) -> {ok, {m_rsc:resource_id(), integer()}} | {error, term()}.
...
set_autologon_cookie(UserId, Context) ->
    ...
    z_context:set_cookie(?AUTOLOGON_COOKIE, Cookie, Options, Context).
...
reset_autologon_cookie(Context) ->
    ...
    z_context:set_cookie(?AUTOLOGON_COOKIE, <<>>, Options, Context).
