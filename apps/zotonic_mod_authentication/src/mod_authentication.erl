...
init(Context) ->
    ...
    case m_config:get(?MODULE, password_min_length, Context) of
        undefined ->
            m_config:set_value(?MODULE, password_min_length, "8", Context);
        _ ->
            nop
    end,
    ok.
...
observe_request_context(#request_context{ phase = init }, Context, _Context) ->
    case z_context:get(anonymous, Context, false) of
        ...
        false ->
            ...
            z_notifier:foldl(#session_context{
                                 request_type = http,
                                 payload = undefined
                             },
                             Context2,
...
observe_auth_options_update(#auth_options_update{ request_options = ROpts }, AccOpts, Context) ->
    case ROpts of
        ...
        #{
            ...
            <<"acl_user_groups_state">> := State
        } ->
            case {m_acl_rule:is_valid_code(SignedCode, Context), State} of
                {true, <<"edit">>} ->
                    AccOpts#{ acl_user_groups_state => edit };
                {true, <<"publish">>} ->
                    maps:remove(acl_user_groups_state, AccOpts);
                {false, _} ->
                    AccOpts
...
observe_logon_options(#logon_options{
    ...
    case z_string:to_lower(
             z_string:trim(Username))
    of
        <<>> ->
            Acc;
        UsernameOrEmail ->
            IsUserLocal =
                is_user_local(UsernameOrEmail, Context)
...
observe_auth_client_logon_user(#auth_client_logon_user{
                                   user_id = UserId,
                                   url = Url
                               },
                               Context) ->
    case z_context:client_topic(Context) of
        {ok, ClientTopic} ->
            case z_authentication_tokens:encode_onetime_token(UserId, Context) of
                {ok, Token} ->
                    z_mqtt:publish(ClientTopic ++ [<<"model">>, <<"auth">>, <<"post">>, <<"onetime-token">>],
                                   #{
                                       token => Token,
                                       url => Url
                                   },
        ...
        _ ->
            case z_acl:is_admin(Context) of
                true ->
                    case z_context:client_topic(Context) of
                        {ok, ClientTopic} ->
                            z_mqtt:publish(ClientTopic ++ [<<"model">>, <<"auth">>, <<"post">>, <<"switch-user">>],
                                           #{ user_id => UserId },
                                           Context),
                            ok;
                        {error, _} = Error ->
...
update_identity(Auth, IdnPs, Context) ->
    ...
    {rsc_id, UserId} = proplists:lookup(rsc_id, IdnPs),
    maybe_update_identity(IdnPropb, Auth#auth_validated.service_props, IdnPs, Context),
    {ok, UserId}.

maybe_signup(Auth, Context) ->
    Email = maps:get(<<"email">>, Auth#auth_validated.props, undefined),
    case not Auth#auth_validated.is_connect andalso is_user_email_exists(Email, Context) of
        true ->
        ...
        false ->
            ...
            case z_notifier:first(Signup, Context) of
                {ok, NewUserId} ->
                    case auth_identity(Auth, Context) of
                        undefined ->
                            insert_identity(NewUserId, Auth, Context);
                        _ ->
                            nop
                    end,
...
insert_identity(UserId, Auth, Context) ->
    ...
    Props = [{is_unique, true}, {is_verified, true}, {propb, {term, Auth#auth_validated.service_props}}],
    m_identity:insert(UserId, Type, Key, Props, Context).

auth_identity(#auth_validated{
                  service = Service,
                  service_uid = Uid
              },
              Context) ->
