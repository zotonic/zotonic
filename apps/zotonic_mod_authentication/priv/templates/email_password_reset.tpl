...
{% if not id %}
    ...
    <p>{_ You've requested a new password for _} <a href="{% url home absolute_url %}">{{ m.site.hostname }}</a>.</p>
...
{% else %}
    ...
    <p>{_ You've requested a new password for _} <a href="{%%}">{{ m.site.hostname }}</a>. {_ Below are your account details and a link to set a new password. _}</p>
