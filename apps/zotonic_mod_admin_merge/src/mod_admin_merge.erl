    ...
    Cats =
        case Category of
            <<>> ->
                [];
            CatId ->
                [{z_convert:to_integer(CatId)}]
        end,
    Vars =
        [{id,
          m_rsc:rid(
              z_context:get_q(<<"id">>, Context), Context)},
         {cat, Cats},
         {text, Text}],
...
event(#submit{ message = {merge, Args} }, Context) ->
    ...
    IsMergeTrans =
        z_convert:to_bool(
            z_context:get_q("is_merge_trans", Context)),
...
merge(WinnerId, _LoserId, <<"merge_only">>, _IsMergeTrans, Context) ->
    case z_acl:rsc_editable(WinnerId, Context) of
        ...
        true ->
            ...
            %     end),
            z_render:wire([{growl, [{text, ?__("not implemented yet", Context)}]}, {dialog_close, []}], Context)
    end;
merge(WinnerId, LoserId, <<"merge_delete">>, IsMergeTrans, Context) ->
    ...
    of
        {true, _, _} ->
            z_render:wire({alert,
                           [{text, ?__("The loser is protected, unprotect the loser before merging.", Context)}]},
                          Context);
        {_, false, _} ->
        ...
        {false, true, true} ->
            ...
            erlang:spawn(fun() ->
                            ...
                            z_notifier:first(#page_actions{
                                                 actions = [{redirect, [{dispatch, admin_edit_rsc}, {id, WinnerId}]}]
                                             },
                                             Context)
                         end),
            z_render:wire([{growl, [{text, ?__("Merging the two pages ...", Context)}]}, {dialog_close, []}], Context)
    end;
