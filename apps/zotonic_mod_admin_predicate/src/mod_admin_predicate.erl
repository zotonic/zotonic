...
event(#submit{ message = {delete_move, Args} }, Context) ->
    ToPredId =
        z_convert:to_integer(
            z_context:get_q_validated(<<"predicate_id">>, Context)),
    ...
    case z_acl:rsc_deletable(PredId, Context) of
        true ->
            ...
            of
                ...
                false ->
                    z_render:growl(?__("Sorry, you are not allowed to insert connections with this predicate.",
                                       Context),
                                   Context)
            end;
        false ->
...
event(#postback{ message = {delete_all, Args} }, Context) ->
    ...
    case not IfEmpty orelse not m_predicate:is_used(PredId, Context) of
        ...
        false ->
            z_render:wire({alert,
                           [{message, ?__("Delete is canceled, there are connections with this predicate.", Context)}]},
                          Context)
    end.
...
observe_rsc_update(#rsc_update{ id = Id }, {ok, Props}, Context) ->
    case maps:is_key(<<"predicate_subject_list">>, Props) orelse maps:is_key(<<"predicate_object_list">>, Props) of
        true ->
            ...
            Props1 = maps:without([<<"predicate_subject_list">>, <<"predicate_object_list">>], Props),
            {ok, Props1};
        false ->
            {ok, Props}
    end;
