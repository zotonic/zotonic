            ...
            {% with app.id as id %}
                <tr id="{{ #tr.id }}" title="{{ app.application_descr|escape }}" data-href="#">
                    ...
                    <td>
                        ...
                        <a class="btn btn-default btn-xs" href="{% url admin_oauth_tokens app_id=id %}" title=_"Show applications/users that are using this key">{_ Users _}</a>
                        {% button class="btn btn-default btn-xs" text=_"Delete" postback={start_del_app id=id} %}
                    </td>
                </tr>
                {% wire id=#tr.id action={redirect dispatch=`admin_oauth_tokens` app_id=id} %}
            {% endwith %}
        {% empty %}
            <tr>
                <td colspan="4">
