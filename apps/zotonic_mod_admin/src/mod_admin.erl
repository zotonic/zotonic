...
-export([observe_sanitize_element/3,
...
admin_menu_content_queries(Context) ->
    #search_result{ result = Result } =
        z_search:search(<<"all_bytitle">>, #{ <<"cat">> => admin_content_query }, 1, 100, Context),
    AdminOverviewQueryId = m_rsc:rid(admin_overview_query, Context),
    Result1 =
        lists:filter(fun({_Title, Id}) ->
                        z_acl:rsc_visible(Id, Context) andalso Id =/= AdminOverviewQueryId
                     end,
                     Result),
    CQ = lists:map(fun({Title, Id}) ->
                      #menu_item{
                          id = {admin_query, Id},
                          parent = admin_content,
                          label = Title,
...
observe_admin_edit_blocks(#admin_edit_blocks{  }, Menu, Context) ->
    [{1,
      ?__("Standard", Context),
      [{header, ?__("Header", Context)}, {text, ?__("Text", Context)}, {page, ?__("Embed page", Context)}]}
     | Menu].
...
event(#postback_notify{ message = <<"admin-insert-block">> }, Context) ->
    ...
    Type =
        z_string:to_name(
            z_context:get_q("type", Context)),
    RscId =
        z_convert:to_integer(
            z_context:get_q("rsc_id", Context)),
    Render =
        #render{
            ...
            vars =
                [{id, RscId},
                 ...
                 {blocks,
                  lists:sort(
                      z_notifier:foldl(#admin_edit_blocks{ id = RscId }, [], Context))}]
        },
    case z_html:escape(
             z_context:get_q("after", Context))
    of
        undefined ->
            z_render:insert_top("edit-blocks", Render, Context);
        AfterId ->
            z_render:insert_after(AfterId, Render, Context)
    end;
event(#postback_notify{
    ...
    CreatorId =
        z_convert:to_integer(
            z_context:get_q(<<"find_creator_id">>, Context)),
    SubjectId =
        z_convert:to_integer(
            z_context:get_q(<<"subject_id">>, Context)),
    ObjectId =
        z_convert:to_integer(
            z_context:get_q(<<"object_id">>, Context)),
    Predicate =
        z_convert:to_binary(
            z_context:get_q(<<"predicate">>, Context, <<>>)),
    ...
    Category =
        case z_context:get_q(<<"find_category">>, Context) of
            undefined ->
    ...
    Cats =
        case z_convert:to_binary(Category) of
            <<"p:", Predicate/binary>> ->
    ...
    Vars =
        [{intent, z_context:get_q(<<"intent">>, Context)},
         ...
         {is_zlink,
          z_convert:to_bool(
              z_context:get_q(<<"is_zlink">>, Context))}]
        ++ case z_context:get_q(<<"find_cg">>, Context) of
               ...
               undefined ->
                   [];
               CgId ->
                   [{content_group, m_rsc:rid(CgId, Context)}]
           end,
    case Trigger of
        <<"dialog-connect-find">> ->
            z_render:wire([{remove_class, [{target, TargetId}, {class, "loading"}]},
                           {update,
                            [{target, TargetId}, {template, "_action_dialog_connect_tab_find_results.tpl"} | Vars]}],
                          Context);
        <<"dialog-new-rsc-tab">> ->
            z_render:wire([{remove_class, [{target, TargetId}, {class, "loading"}]},
                           {update,
                            [{target, TargetId}, {template, "_action_dialog_new_rsc_tab_find_results.tpl"} | Vars]}],
                          Context)
    end;
event(#postback{ message = {admin_connect_select, Args} }, Context) ->
    ...
    IsConnected =
        z_convert:to_bool(
            z_context:get_q(<<"is_connected">>, Context)),
    ...
    IsConnectToggle =
        z_convert:to_bool(
            proplists:get_value(is_connect_toggle, Args)),
    ...
    {SubjectId, ObjectId} =
        case z_utils:is_empty(ObjectId0) of
            true ->
                {z_convert:to_integer(SubjectId0), z_convert:to_integer(SelectId)};
            false ->
                {z_convert:to_integer(SelectId), z_convert:to_integer(ObjectId0)}
        end,

    % Only disconnect if connection is not made from tinymce
    IsUnlink =
        case IsConnectToggle of
            false ->
                false;
            true ->
                IsConnected
        ...
        {ok, Context1} ->
            Context2 =
                case z_convert:to_bool(
                         proplists:get_value(autoclose, Args))
                of
                    true ->
...
event(#postback{ message = {admin_rsc_redirect, Args} }, Context) ->
    SelectId =
        m_rsc:rid(
            z_context:get_q(<<"select_id">>, Context), Context),
    Dispatch =
        case proplists:get_value(redirect, Args) of
            true ->
                admin_edit_rsc;
            false ->
                admin_edit_rsc;
    ...
    case Callback of
        ...
        CB ->
            {ok,
             z_render:wire({script,
                            [{script,
                              [z_sanitize:ensure_safe_js_callback(CB), $(, z_utils:js_object(Vars, Context), $), $;]}]},
                           Context)}
    end;
        ...
        true ->
            case m_edge:get_id(SubjectId, Predicate, ObjectId, Context) of
                undefined when IsUnlink ->
                    do_link_unlink_feedback(false, false, undefined, SubjectId, Predicate, ObjectId, Callback, Context);
                undefined when not IsUnlink ->
                    case m_edge:insert(SubjectId, Predicate, ObjectId, Context) of
                        {ok, EdgeId} ->
                            do_link_unlink_feedback(true,
                                                    false,
                                                    EdgeId,
                                                    SubjectId,
                                                    Predicate,
                ...
                EdgeId when not IsUnlink ->
                    do_link_unlink_feedback(false, false, EdgeId, SubjectId, Predicate, ObjectId, Callback, Context)
            end;
        false ->
            do_link_unlink_error(IsUnlink, Context)
    end.
...
do_link_unlink_feedback(IsNew, IsDelete, EdgeId, SubjectId, Predicate, ObjectId, Callback, Context) ->
    ContextP = context_language(Context),
    Title =
        case m_rsc:p(ObjectId, title, Context) of
            undefined ->
                ?__("Untitled", Context);
            T ->
                T
    ...
    Context1 =
        case {IsNew, IsDelete} of
            {true, false} ->
                z_render:growl([?__("Added the connection to", ContextP), <<" \"">>, Title, <<"\".">>], Context);
            {false, true} ->
                z_render:growl([?__("Removed the connection to", ContextP), <<" \"">>, Title, <<"\".">>], Context);
            {false, false} ->
                Context
        end,
    case Callback of
    ...
    case z_context:get_q("language", Context) of
        ...
        Lang ->
            case z_language:to_language_atom(Lang) of
                {ok, LanguageCode} ->
                    z_context:set_language(LanguageCode, Context);
                _ ->
                    Context
            end
    end.
...
manage_schema(_Version, _Context) ->
    #datamodel{
        categories =
            [{admin_content_query,
              query,
              [{title, {trans, [{en, <<"Admin content query">>}, {nl, <<"Admin inhoud zoekopdracht">>}]}}]}]
    }.
